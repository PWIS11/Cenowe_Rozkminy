...
## Walidacja wstępna danych i określenie reguł logicznych
...
p1 <- ggplot(dane_filtr, aes(x = has_balcony, y = price)) +
  ...
  theme(
    ...
    axis.text = element_text(size = 10)  # Czcionka etykiet osi
  )
...
```{r}
# 1. test chi-kwadrat opisanie dobrze 
# niżej kod do chi-kwadrat
# 2. shapirowilka wynonujesz dla max 5 tys obserwacji, bada rozład normalny (normalność) danych a matematycznie rozkłady przyjmują parametry liczbowe zeby np wyznaczyc funkcje gęstości więc robimy go tylko na danych ilościowych. dla zmiennych jakosciowych nie ma to sensu
# 3. możemy również zbadać jednorodność wariancji ponieważ test manna wykonujemy bo nie mamy rozkładu normalnego i równych wariancji, 
```
...
tabela <- table(dane_filtr$has_balcony, dane_filtr$has_elevator)
chisq.test(tabela)
```
...
# Lista zmiennych do porównania
categorical_vars <- c("has_balcony", "has_parking", "has_elevator", "has_security", "has_storage_room")

# Sprawdzenie, które zmienne istnieją w zbiorze danych
valid_vars <- categorical_vars[categorical_vars %in% colnames(dane)]

# Informacja o zmiennych do analizy
cat("\nZmiennie do analizy:", valid_vars, "\n")

# Funkcja do wykonania testu Chi-kwadrat dla każdej pary zmiennych
chi_square_results <- list()

for (i in 1:(length(valid_vars) - 1)) {  # Iteracja do przedostatniej zmiennej
  for (j in (i+1):length(valid_vars)) {  # Iteracja od następnej zmiennej
    var1 <- valid_vars[i]
    var2 <- valid_vars[j]
    
    # Walidacja obecności zmiennych w danych
    if (var1 %in% colnames(dane) & var2 %in% colnames(dane)) {
      # Tworzenie tymczasowego zbioru danych z wybranymi zmiennymi
      temp_data <- dane[, c(var1, var2), drop = FALSE]
      
      # Usunięcie wierszy z brakującymi wartościami (NA)
      temp_data <- na.omit(temp_data)
      
      # Sprawdzenie, czy obie zmienne mają dane
      if (nrow(temp_data) > 0) {
        # Tworzenie tablicy kontyngencji
        contingency_table <- table(temp_data[[var1]], temp_data[[var2]])
        
        # Test Chi-kwadrat (jeśli tablica kontyngencji ma odpowiedni rozmiar)
        if (min(dim(contingency_table)) > 1) {
          ...
          cat("\nTablica kontyngencji dla", var1, "vs", var2, "jest za mała, aby przeprowadzić test.\n")
        }
      } else {
        cat("\nBrak danych do analizy dla", var1, "vs", var2, "\n")
      }
    } else {
      cat("\nJedna z kolumn (", var1, "lub", var2, ") nie istnieje w danych.\n")
    }
  }
}

# Ramka danych z wynikami testów Chi-kwadrat
chi_square_results_df <- data.frame(
  Comparison = c(
    "has_balcony vs has_parking",
  ...
  Chi_squared_Statistic = c(
    178.6818, 21.27356, 139.2903, 160.8436,
    208.3458, 79.08341, 48.34977, 282.721,
    1775.337, 332.2429
  ),
  P_value = c(
    9.402443e-41, 3.981856e-06, 3.805473e-32, 7.401809e-37,
    3.153351e-47, 5.954214e-19, 3.565859e-12, 1.917139e-63,
    1e-300, 3.120911e-74
  ),
  Degrees_of_Freedom = rep(1, 10) # Wszystkie testy mają df = 1
)

# Formatowanie P-value w notacji naukowej
chi_square_results_df$P_value <- format(chi_square_results_df$P_value, scientific = TRUE, digits = 3)

kable(chi_square_results_df, caption = "Wyniki testów Chi-kwadrat")
```
...
1.  **`has_balcony` vs `has_parking`**
    -   Tablica kontyngencji pokazuje, że mieszkania z balkonem są
        bardziej skorelowane z posiadaniem parkingu.
    -   Statystyka Chi-kwadrat: **178.68**
    -   Wartość p: $9.40 \times 10^{-41}$ (bardzo niskie)
    -   Wnioski: Istnieje **silna zależność** między posiadaniem balkonu
        a parkingu.
2.  **`has_balcony` vs `has_elevator`**
    -   Wynik wskazuje na zależność między balkonem a windą.
    -   Statystyka Chi-kwadrat: **21.27**
    -   Wartość p: $3.98 \times 10^{-6}$
    -   Wnioski: Istnieje **istotna zależność**, ale słabsza niż w
        innych przypadkach.
3.  **`has_balcony` vs `has_security`**
    -   Statystyka Chi-kwadrat: **139.29**
    -   Wartość p: $3.80 \times 10^{-32}$
    -   Wnioski: Istnieje **bardzo silna zależność** między balkonem a
        obecnością zabezpieczeń.
4.  **`has_balcony` vs `has_storage_room`**
    -   Statystyka Chi-kwadrat: **160.84**
    -   Wartość p: $7.40 \times 10^{-37}$
    -   Wnioski: Zależność między balkonem a komórką lokatorską jest
        **bardzo silna i istotna**.
5.  **`has_parking` vs `has_elevator`**
    -   Statystyka Chi-kwadrat: **208.35**
    -   Wartość p: $3.15 \times 10^{-47}$
    -   Wnioski: Istnieje **silna zależność** między parkingiem a windą.
6.  **`has_parking` vs `has_security`**
    -   Statystyka Chi-kwadrat: **79.08**
    -   Wartość p: $5.95 \times 10^{-19}$
    -   Wnioski: Parking i zabezpieczenia są również **istotnie
        zależne**, ale w mniejszym stopniu.
7.  **`has_parking` vs `has_storage_room`**
    -   Statystyka Chi-kwadrat: **48.35**
    -   Wartość p: $3.56 \times 10^{-12}$
    -   Wnioski: Zależność między parkingiem a komórką lokatorską jest
        **istotna, ale umiarkowana**.
8.  **`has_elevator` vs `has_security`**
    -   Statystyka Chi-kwadrat: **282.72**
    -   Wartość p: $1.92 \times 10^{-63}$
    -   Wnioski: Winda i zabezpieczenia są **silnie i istotnie
        skorelowane**.
9.  **`has_elevator` vs `has_storage_room`**
    -   Statystyka Chi-kwadrat: **1775.34**
    -   Wartość p: **0** (najsilniejszy możliwy wynik)
    -   Wnioski: Winda i komórka lokatorska są **bardzo silnie
        zależne**.
10. **`has_security` vs `has_storage_room`**
    -   Statystyka Chi-kwadrat: **332.24**
    -   Wartość p: $3.12 \times 10^{-74}$
    -   Wnioski: Obecność zabezpieczeń i komórki lokatorskiej jest
        **silnie i istotnie zależna**.

#### Ogólne wnioski:

Wszystkie pary zmiennych kategorycznych wykazują istotne zależności, co
sugeruje, że cechy mieszkań są często powiązane. Najsilniejsze
zależności zaobserwowano w przypadku: - `has_elevator` vs
`has_storage_room` ($\chi^2 = 1775.34$), - `has_security` vs
`has_storage_room` ($\chi^2 = 332.24$).

Zależności te mogą wynikać z projektowania mieszkań w określonych
standardach lub preferencji rynkowych.

### Testowanie normalności i wybór odpowiedniego testu statystycznego

#### Wprowadzenie

# SHAPIRO TYLKO ZMIENNE ILOŚCIOWEEE (JAKOŚCIOWE NIE MOGĄ MIEĆ ROZKŁADU NORMALNEGO - NIE POLICZYSZ WARIANCJI ANI W.OCZEKIWANEJ Z TAKICH)

Celem analizy jest sprawdzenie, czy istnieją istotne różnice w średnich
wartości zmiennej `price` pomiędzy grupami (`yes` i `no`) dla wybranych
zmiennych kategorycznych (`has_balcony`, `has_parking`, `has_elevator`,
`has_security`, `has_storage_room`). Aby dobrać odpowiednią metodę
testowania, najpierw musimy ocenić, czy dane spełniają założenia testu
t-Studenta, w tym normalność rozkładu w grupach.

#### Ograniczenia testu Shapiro-Wilka

Test Shapiro-Wilka, mimo swojej skuteczności w badaniu normalności, ma
ograniczenia:

1.  **Minimalna liczba obserwacji:** Wymaga co najmniej 3 obserwacji w
    każdej grupie.

2.  **Maksymalna liczba obserwacji:** Nie może być stosowany dla prób
    większych niż 5000 obserwacji.

W przypadku dużych prób (więcej niż 5000 obserwacji w grupie) konieczne
było zastosowanie losowego próbkowania, aby dostosować dane do wymagań
testu.

#### Zastosowanie losowego próbkowania

Dla grup zawierających więcej niż 5000 obserwacji wylosowano dokładnie
5000 elementów. Dzięki temu test Shapiro-Wilka mógł być przeprowadzony
na reprezentatywnych próbkach:

-   **Zastosowanie próbkowania losowego:** Zapewnia, że wyniki są zgodne
    z charakterystyką całej grupy.

-   **Powtarzalność wyników:** Użyto funkcji `set.seed(123)` w celu
    zapewnienia powtarzalności losowania.

#### Wyniki analizy

Wyniki testu Shapiro-Wilka zostały zapisane w tabeli, która zawiera
następujące informacje:

-   **Zmienna (`Variable`):** Nazwa zmiennej kategorycznej.

-   **Grupa (`Group`):** Grupa `yes` lub `no`.

-   **Statystyka testu (`Shapiro_Statistic`):** Wartość statystyki
    Shapiro-Wilka.

-   **P-value (`P_Value`):** Wartość p, wskazująca istotność
    statystyczną.

-   **Liczba obserwacji (`Observations`):** Liczba obserwacji użytych do
    testu (po próbkowaniu, jeśli zastosowano).

```{r Shapiro-Wilk, echo=FALSE, message=FALSE}
# Lista zmiennych kategorycznych do analizy
categorical_vars <- c("has_balcony", "has_parking", "has_elevator", "has_security", "has_storage_room")

# Tworzymy pustą ramkę danych na wyniki testu Shapiro-Wilka
shapiro_results <- data.frame(
  Group = character(),
  Variable = character(),
  Shapiro_Statistic = numeric(),
  P_Value = numeric(),
  Observations = integer(),
  stringsAsFactors = FALSE
)

# Maksymalna liczba obserwacji dla testu Shapiro-Wilka
max_obs <- 5000

# Iteracja po zmiennych kategorycznych
for (var in categorical_vars) {
  for (group in c("yes", "no")) { # Grupy to "yes" i "no"
    group_data <- dane$price[dane[[var]] == group]  # Pobieramy dane dla grupy
    
    # Liczba obserwacji w grupie
    n_obs <- length(group_data)
    
    # Jeżeli liczba obserwacji przekracza 5000, losujemy próbkę
    if (n_obs > max_obs) {
      set.seed(123)  # Dla powtarzalności wyników
      group_data <- sample(group_data, max_obs)
      n_obs <- max_obs
    }
    
    # Test Shapiro-Wilka dla odpowiednich grup
    if (n_obs >= 3) {
...
# Formatowanie P-value w notacji naukowej
shapiro_results$P_Value <- format(shapiro_results$P_Value, scientific = TRUE, digits = 3)


kable(shapiro_results, caption = "Wyniki testu Shapiro-Wilka dla zmiennej price w grupach")
```

#### Interpretacja wyników testu Shapiro-Wilka dla zmiennej `price`

Celem analizy było sprawdzenie, czy rozkład zmiennej `price` w grupach
(`yes` i `no`) dla wybranych zmiennych kategorycznych spełnia założenie
normalności. Poniżej przedstawiono interpretację wyników dla każdej
zmiennej.
...
1.  **Wartość p (`P_Value`)**:
    ...
    -   Dla wszystkich testowanych grup (`yes` i `no`) wartość p jest
        **mniejsza niż 0.05**.

    -   Odrzucamy hipotezę zerową testu Shapiro-Wilka, co oznacza, że
        rozkład danych **istotnie odbiega od normalności** w każdej
        grupie.

2.  **Statystyka Shapiro-Wilka (`Shapiro_Statistic`)**:

    -   Statystyka Shapiro-Wilka jest zbliżona do 1, co oznacza, że dane
        mają pewne podobieństwa do rozkładu normalnego, ale wynik testu
        wskazuje na istotne odchylenia.

3.  **Liczba obserwacji (`Observations`)**:

    -   Większość grup miała 5000 obserwacji, co wynika z zastosowania
        próbkowania dla grup większych niż 5000.

    -   Wyjątek: `has_security` w grupie `yes`, która miała 2661
        obserwacji (pełna liczba w danych).

#### Wyniki dla poszczególnych zmiennych:

1.  **`has_balcony`**:

    -   **Grupa `yes`:** Statystyka Shapiro-Wilka: 0.994, P-value:
        1.22×10−13.

    -   **Grupa `no`:** Statystyka Shapiro-Wilka: 0.994, P-value:
        5.62×10−13.

    -   **Wniosek:** Dane w obu grupach istotnie odbiegają od
        normalności.
...
2.  **`has_parking`**:
    ...
    -   **Grupa `no`:** Statystyka Shapiro-Wilka: 0.993, P-value:
        2.61×10−15.

    -   **Wniosek:** Dane w obu grupach odbiegają od normalności.

3.  **`has_elevator`**:

    -   **Grupa `yes`:** Statystyka Shapiro-Wilka: 0.991, P-value:
        1.63×10−17.
...
4.  **`has_security`**:
    ...
    -   **Grupa `yes`:** Statystyka Shapiro-Wilka: 0.996, P-value:
        5.52×10−6.

    -   **Grupa `no`:** Statystyka Shapiro-Wilka: 0.990, P-value:
        3.52×10−18.

    -   **Wniosek:** Dane odbiegają od normalności w obu grupach, choć w
        grupie `yes` odchylenie jest mniej wyraźne (wyższa wartość
        statystyki i p-value).

5.  **`has_storage_room`**:

    -   **Grupa `yes`:** Statystyka Shapiro-Wilka: 0.990, P-value:
        1.98×10−18.

    -   **Grupa `no`:** Statystyka Shapiro-Wilka: 0.996, P-value:
        6.33×10−11.
...
1.  **Rozkład zmiennej `price` odbiega od normalności w każdej grupie
    (`yes` i `no`) dla wszystkich analizowanych zmiennych
    kategorycznych.**

2.  **Zalecenia dotyczące dalszej analizy:**

    -   **Test t-Studenta:** Można rozważyć jego użycie, jeśli próbki są
        duże i opieramy się na Centralnym Twierdzeniu Granicznym (CTG).

    -   **Test Wilcoxona (Mann-Whitney U):** Ze względu na brak
        normalności, test Wilcoxona może być bardziej odpowiedni do
        porównania grup.
...
#  ANDERSON WYŻEJ ???? O NIM TEŻ TRZEBA WSPOMNIEĆ PO TO BYŁ ROBIONY

Na podstawie wyników testu Shapiro-Wilka stwierdzono, że rozkład
zmiennej `price` odbiega od normalności w każdej grupie (`yes` i `no`)
dla wszystkich analizowanych zmiennych kategorycznych. W związku z tym,
zamiast testu t-Studenta, zdecydowano się zastosować **test
Mann-Whitneya (Wilcoxona)**, który nie wymaga założenia normalności
rozkładu danych.

#### Cel testu Mann-Whitneya

# ten test nie wymaga normalnosci i jednorodnosci wariancji. 

Test Mann-Whitneya (zwany również testem Wilcoxona) bada, czy mediany
zmiennej `price` różnią się istotnie pomiędzy grupami `yes` i `no` dla
każdej zmiennej kategorycznej. Jest to test nieparametryczny, który jest
bardziej odporny na odchylenia od założeń normalności niż test
t-Studenta.

# mann

```{r}
krakow <- dane_filtr[dane_filtr$city == "Kraków", ]
winda_krakow <- wilcox.test(krakow$price ~ krakow$has_elevator, alternative = "less") # mozesz wybrac alternative np less albo jeszcze inne

#Wartość p < 0.05  wynik statystycznie istotny. Możemy odrzucić hipotezę zerową (czyli brak różnicy) na poziomie istotności 0.05.
# H0 -> ceny mieszkań z windą i bez windy są takie same.
# H1 ->  ceny mieszkań z windą są mniejsze niż ceny mieszkań bez windy. alternative = "less" oznacza, że zakłada się, iż grupa "z windą" ma mniejsze ceny.

#drzucamy hipotezę zerową, co oznacza, że istnieje różnica pomiędzy cenami mieszkań z windą a cenami mieszkań bez windy.
#przez alternative = less interpretacja taka ->  wynik sugeruje, że mieszkania z windą mają niższe ceny niż te bez windy
```
...
# Tworzymy pustą ramkę danych na wyniki testu Mann-Whitneya
...
# Iteracja po zmiennych kategorycznych
for (var in categorical_vars) {
  if (var %in% colnames(dane)) {
    # Dane dla grup yes i no
    group_yes <- dane$price[dane[[var]] == "yes"]
    group_no <- dane$price[dane[[var]] == "no"]
    
    # Test Mann-Whitneya
    wilcox_test <- wilcox.test(group_yes, group_no, exact = FALSE) # exact=FALSE dla dużych prób
    
    # Obliczanie median
    median_yes <- median(group_yes, na.rm = TRUE)
    median_no <- median(group_no, na.rm = TRUE)
    
    # Zapis wyników
    mann_whitney_results <- rbind(mann_whitney_results, data.frame(
      Variable = var,
...
# Formatowanie P-value w notacji naukowej
mann_whitney_results$P_Value <- format(mann_whitney_results$P_Value, scientific = TRUE, digits = 3)

kable(mann_whitney_results, caption = "Wyniki testu Mann-Whitneya dla zmiennej price w grupach")
```
...
Test Mann-Whitneya został przeprowadzony, aby sprawdzić, czy mediany
zmiennej `price` różnią się istotnie pomiędzy grupami `yes` i `no` dla
każdej zmiennej kategorycznej. Wyniki testu przedstawiono poniżej.

#### Kluczowe wyniki:

1.  **`has_balcony`:**
    -   **Wartość statystyki W:** 62,353,150
    -   **P-value:** $2.05 \times 10^{-44}$ (bardzo niskie, istotne
        statystycznie)
    -   **Mediana (yes):** 13.52649
    -   **Mediana (no):** 13.44300
    -   **Wniosek:** Istnieje istotna statystycznie różnica w medianie
        `price` pomiędzy mieszkaniami z balkonem (`yes`) a bez balkonu
        (`no`). Mieszkania z balkonem mają wyższą medianę ceny.
2.  **`has_parking`:**
    -   **Wartość statystyki W:** 55,741,107
    -   **P-value:** $7.32 \times 10^{-118}$ (bardzo niskie, istotne
        statystycznie)
    -   **Mediana (yes):** 13.59112
    -   **Mediana (no):** 13.45167
    -   **Wniosek:** Mieszkania z parkingiem (`yes`) mają istotnie
        wyższą medianę ceny w porównaniu do mieszkań bez parkingu
        (`no`).
3.  **`has_elevator`:**
    -   **Wartość statystyki W:** 71,840,735
    -   **P-value:** $1.48 \times 10^{-219}$ (bardzo niskie, istotne
        statystycznie)
    -   **Mediana (yes):** 13.57710
    -   **Mediana (no):** 13.39848
    -   **Wniosek:** Mieszkania z windą (`yes`) mają istotnie wyższą
        medianę ceny w porównaniu do mieszkań bez windy (`no`).
4.  **`has_security`:**
    -   **Wartość statystyki W:** 31,442,538
    -   **P-value:** $2.00 \times 10^{-100}$ (bardzo niskie, istotne
        statystycznie)
    -   **Mediana (yes):** 13.66352
    -   **Mediana (no):** 13.45741
    -   **Wniosek:** Obecność zabezpieczeń (`yes`) wiąże się z istotnie
        wyższą medianą ceny mieszkań w porównaniu do mieszkań bez
        zabezpieczeń (`no`).
5.  **`has_storage_room`:**
    -   **Wartość statystyki W:** 45,223,376
    -   **P-value:** $9.00 \times 10^{-146}$ (bardzo niskie, istotne
        statystycznie)
    -   **Mediana (yes):** 13.41354
    -   **Mediana (no):** 13.55415
    -   **Wniosek:** W przeciwieństwie do innych zmiennych, mieszkania z
        komórką lokatorską (`yes`) mają **niższą medianę ceny** w
        porównaniu do mieszkań bez komórki (`no`).

#### Ogólne wnioski:

1.  **Istotne różnice w medianach:**
    -   Dla wszystkich zmiennych kategorycznych różnice w medianach
        `price` pomiędzy grupami `yes` i `no` są statystycznie istotne
        (p-value \< 0.05).
2.  **Wpływ cech na medianę ceny:**
    -   Cechy takie jak balkon, parking, winda, czy zabezpieczenia są
        związane z wyższymi medianami cen mieszkań.
    -   **Wyjątek:** Komórka lokatorska (`has_storage_room`), gdzie
        mediany wskazują na niższą cenę mieszkań z tą cechą.

# **Analiza czynników geogaficznych wpływających na ceny mieszkań**
...
3.  **Białystok, Rzeszów**:
    -   W przypadku Rzeszowa widzimy, że linia jest nachylona w górę,
        ...
        na cenę mieszkań.
4.  **Rozrzut danych**:
    -   Dane są gęsto skupione blisko początku osi X (odległość do 0,5–2
        km). To sugeruje, że większość mieszkań znajduje się blisko
        przedszkoli. W miastach takich jak Kraków i Warszawa dane są
        bardziej rozproszone w porównaniu z mniejszymi miastami.
...
1.  **Warszawa i Kraków**
    -   W **Warszawie** i **Krakowie** zauważalne jest, że odległość od
        restauracji ma delikatnie **ujemny wpływ na ceny mieszkań** – im
        dalej restauracji, tym ceny mieszkań są niższe To może
