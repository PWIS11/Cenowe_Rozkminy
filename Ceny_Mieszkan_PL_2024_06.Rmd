...
po wstępnym oczyszczeniu danych. Jej celem jest sprawdzenie zgodności danych 
z określonymi regułami logicznymi i jakościowymi, aby upewnić się, że dane 
są kompletne, spójne <br>i gotowe do dalszej analizy oraz modelowania.

Proces ten polega na weryfikacji, czy dane spełniają określone kryteria, 
zapewniając ich poprawność, spójność oraz integralność. Szczególną uwagę 
zwraca się na typy danych i analizę unikalnych wartości w poszczególnych kolumnach. 
Każda kolumna powinna mieć odpowiedni typ danych (np. numeryczny, tekstowy, całkowity),
a liczba unikalnych wartości powinna być zgodna <br> z oczekiwaniami.

Dzięki takiej analizie można wykryć potencjalne błędy w strukturze danych, 
takie jak niewłaściwe przypisanie typu do kolumny czy nieoczekiwane wartości. 
Zapewnia to odpowiednie przygotowanie danych do dalszych etapów analizy i modelowania.
...
mają dużą liczbę unikalnych wartości, <br> co sugeruje, że są to dane ciągłe,
które będą odpowiednie do analizy regresyjnej. 

- Zmienne **kategoryczne**, takie jak `building_type`, `ownership` czy `building_material`, 
mogą być traktowane jako zmienne typu factor i wykorzystane w analizach
klasyfikacyjnych. 

- Kolumny z typem **integer**, jak np. `price` mogą być również używane 
...
Na podstawie tej analizy możemy przejść do przygotowania danych do modelowania, 
dobierając odpowiednie metody przetwarzania, takie jak kodowanie zmiennych kategorycznych,
normalizację zmiennych numerycznych czy tworzenie nowych zmiennych, 
<br>w zależności od typu danych i celu analizy.
...
Kolejnym krokiem w procesie walidacji jest zastosowanie określonych reguł 
logicznych, które zapewniają spójność danych. 
<br>
<br>**W projekcie zastosowano następujące reguły walidacyjne**:
...
6. Jeżeli liczba kondygnacji (`Floor Count`) wynosi 0, to w takim budynku nie 
powinno być windy (`Has Elevator`).

Dzięki weryfikacji typów danych oraz zastosowaniu reguł walidacyjnych, 
upewniamy się, że dane są zgodne z określonymi kryteriami logicznymi 
i są gotowe do dalszej analizy.
...
W wyniku przeprowadzonej walidacji zauważono, że w zbiorze danych występuje znaczna
liczba **wartości brakujących (NA)**, <br> w niektórych kolumnach.
...
addwwfw$expression <- c("Floor>floor_count", "build_year<=2024","build_year>1600","Floor>=0", "floor_count>= 0", "Rooms>0", "Rooms<= 15", "Elevator in a building with 0 floors")
...
kondygnacji po modernizacji). Aby poprawić te błędy, przyjęto zasadę, że
w takich sytuacjach wartość liczby kondygnacji (`floor_count`) zostanie
ustawiona na wartość piętra (`floor`). Jeśli wartość liczby kondygnacji
była równa lub większa od wartości piętra, dane pozostały niezmienione.
Taka korekta zapewnia **spójność danych** i eliminuje przypadki, w których
piętro przewyższa liczbę kondygnacji w budynku.**Nie znaleziono duplikatów
wierszy w danych**.
...
mieszkań wyposażonych w to udogodnienie (szczególnie w dużych miastach, 
takich jak Warszawa i Kraków, gdzie dostęp do prywatnej przestrzeni zewnętrznej 
może być bardziej ceniony przez kupujących). Dodatkowo pudełko w tej grupie
(symbolizujące zakres między dolnym a górnym kwartylem) jest węższe,
co oznacza mniejsze zróżnicowanie cen wśród mieszkań z balkonem.
...
różnorodności innych czynników wpływających na cenę, które w tej grupie mają większe znaczenie.

rzut cen (długość pudełek) jest większy dla mieszkań bez parkingu, co sugeruje większą zmienność cen w tej grupie.
...
1. **Parking:** Kluczowy czynnik w dużych miastach, zwłaszcza w Warszawie, gdzie dostępność miejsc parkingowych jest ograniczona.
2. **Ochrona:** Wysoko cenione w dużych aglomeracjach, szczególnie tam, gdzie bezpieczeństwo jest priorytetem.
3. **Komórka lokatorska:** Ważna w miastach o wysokim zagęszczeniu, gdzie przestrzeń przechowywania ma istotne znaczenie.
4. **Balkon:** Istotny czynnik w Warszawie i Krakowie, gdzie dostęp do przestrzeni zewnętrznej jest ograniczony.
5. **Winda:** Znaczący wpływ na wartość mieszkań, szczególnie w dużych miastach z nowoczesną zabudową.

Obecność balkonu może być szczególnie istotna w miastach o wyższej gęstości zabudowy 
(np. Warszawa, Kraków), gdzie dostęp do przestrzeni zewnętrznej, nawet w formie 
niewielkiego balkonu, jest bardziej ceniony. 


#### Interpretacja wyników analizy wpływu obecności windy na cenę mieszkań: 

W analizie porównaliśmy ceny mieszkań w dwóch grupach: z windą i bez windy. 
Obserwacje z wykresów pudełkowych wskazują, że mediana cen w obu grupach jest 
do siebie zbliżona, co sugeruje, że obecność windy nie ma znaczącego wpływu 
na główną tendencję cenową mieszkań. W większości przypadków różnice są minimalne, 
ale mogą występować subtelne różnice w zależności od lokalizacji, 
co może wynikać z dodatkowych czynników wpływających na wartość nieruchomości.

Jednakże mediana w grupie mieszkań z windą jest nieco wyższa w miastach takich 
jak Kraków i Warszawa, co może wskazywać na minimalną przewagę cenową mieszkań 
wyposażonych w to udogodnienie, szczególnie w większych miastach, gdzie dostęp do wygody, 
jaką daje winda, jest bardziej ceniony przez kupujących. Dodatkowo pudełko w tej grupie 
(symbolizujące zakres między dolnym a górnym kwartylem) jest węższe, co oznacza mniejsze
zróżnicowanie cen wśród mieszkań z windą.

Z kolei grupa mieszkań bez windy charakteryzuje się szerszym pudełkiem,
co sugeruje większą zmienność cen. Może to wynikać z różnorodności innych czynników
wpływających na cenę, które w tej grupie mają większe znaczenie. W miastach takich jak Białystok,
Radom czy Rzeszów, gdzie różnice cenowe są subtelne, zmienność cen jest porównywalna w obu grupach, 
co wskazuje na inne czynniki, które mają wpływ na cenę mieszkań.

Obecność windy może być szczególnie istotna w miastach o wyższej gęstości zabudowy, 
jak Warszawa i Kraków, gdzie łatwiejszy dostęp do wyższych pięter, szczególnie 
w blokach mieszkalnych, może wpływać na postrzeganą wartość nieruchomości.


#### Interpretacja wyników analizy wpływu obecności parkingu na cenę mieszkań: 

W analizie porównaliśmy ceny mieszkań w dwóch grupach: z parkingiem i bez parkingu. 
Obserwacje z wykresów pudełkowych wskazują, że wpływ obecności parkingu na ceny mieszkań 
różni się w zależności od lokalizacji. W miastach takich jak Częstochowa i Radom mediana 
cen mieszkań z parkingiem jest wyraźnie wyższa niż w grupie mieszkań bez parkingu. 
Wskazuje to, że w tych lokalizacjach parking jest kluczowym udogodnieniem, które 
znacznie podnosi wartość nieruchomości. 

Z kolei w miastach takich jak Warszawa, Kraków i Rzeszów różnice między medianami 
w obu grupach są niewielkie. Sugeruje to, że obecność parkingu w tych lokalizacjach 
jest standardowym udogodnieniem i ma mniejsze znaczenie w kształtowaniu cen nieruchomości. 
W szczególności w Warszawie i Krakowie, gdzie ceny mieszkań są ogólnie wyższe, większe 
znaczenie mogą mieć inne czynniki, takie jak lokalizacja czy standard wykończenia. 

Dodatkowo, w miastach takich jak Radom i Częstochowa grupa mieszkań „z parkingiem” 
charakteryzuje się węższym pudełkiem (mniejszą zmiennością cen), co sugeruje 
bardziej jednolite ceny w tej grupie. W miastach o niższych medianach (jak Radom) 
obecność parkingu może być cechą premium, szczególnie cenioną przez kupujących.

W miastach takich jak Białystok różnice między medianami są subtelne, 
co wskazuje, że w tych lokalizacjach parking nie jest głównym czynnikiem 
wpływającym na cenę nieruchomości. Jednakże zauważalna przewaga mediany w grupie mieszkań 
z parkingiem może świadczyć o tym, że w wyższych segmentach cenowych obecność parkingu jest dodatkowym atutem.

Obecność parkingu ma szczególnie istotne znaczenie w miastach takich jak Radom i Częstochowa,
gdzie różnice cen między grupami są wyraźne. W dużych aglomeracjach, takich jak Warszawa czy Kraków, 
wpływ parkingu na ceny jest mniej widoczny, co może wynikać z większego znaczenia 
innych czynników wpływających na wartość nieruchomości. 


#### Interpretacja wyników analizy wpływu obecności ochrony na cenę mieszkań: 

W analizie porównaliśmy ceny mieszkań w dwóch grupach: z ochroną i bez ochrony.
Obserwacje z wykresów pudełkowych wskazują, że wpływ obecności ochrony na ceny mieszkań 
jest różnorodny w zależności od lokalizacji. W miastach takich jak Warszawa i Kraków
mediana cen mieszkań z ochroną jest wyraźnie wyższa niż w grupie mieszkań bez ochrony.
Sugeruje to, że w tych dużych aglomeracjach ochrona jest postrzegana jako 
istotne udogodnienie, które wpływa na postrzeganą wartość nieruchomości. 

W miastach takich jak Radom czy Częstochowa różnice między medianami w obu grupach są subtelne. 
Wskazuje to, że obecność ochrony nie odgrywa kluczowej roli w kształtowaniu cen nieruchomości 
w tych lokalizacjach.W Radomiu dodatkowo można zauważyć, że pudełka w obu grupach są podobne 
pod względem szerokości, co oznacza zbliżoną zmienność cen niezależnie od obecności ochrony.

W Rzeszowie różnica median jest zauważalna – mieszkania z ochroną są droższe niż te bez ochrony.
Może to wynikać z faktu, że ochrona jest bardziej pożądanym udogodnieniem w nowoczesnych osiedlach,
które mogą być dominujące w ofercie mieszkań w tej lokalizacji. Jednocześnie pudełko w grupie 
mieszkań „z ochroną” jest nieco szersze, co świadczy o większej zmienności cen w tej grupie.

W miastach takich jak Białystok różnice w medianach są minimalne, co może oznaczać, 
że ochrona nie jest postrzegana jako istotny czynnik wpływający na wartość mieszkania. 
W tych lokalizacjach inne czynniki, takie jak dostępność komunikacyjna czy standard 
wykończenia, mogą odgrywać większą rolę w kształtowaniu cen.

Ochrona ma największy wpływ na ceny mieszkań w miastach takich jak:
Warszawa, Kraków i Rzeszów, gdzie mediana cen w grupie mieszkań z ochroną jest wyraźnie wyższa. 
W mniejszych miastach, takich jak Radom, Częstochowa czy Białystok, różnice w medianach są mniejsze, 
co wskazuje, że obecność ochrony nie jest kluczowym czynnikiem determinującym cenę nieruchomości.
W dużych miastach, gdzie bezpieczeństwo jest istotnym aspektem dla kupujących, 
ochrona wydaje się być bardziej cenionym udogodnieniem.

W dużych miastach, takich jak Warszawa, Kraków i Rzeszów, grupa mieszkań „z ochroną” 
charakteryzuje się większą liczbą wartości odstających (outliers). 
Wskazuje to, że ochrona jest często obecna w droższych i bardziej luksusowych nieruchomościach,
co podnosi górne granice cen.

Z kolei w mniejszych miastach, takich jak Radom, Częstochowa i Białystok,
liczba outliers jest niewielka w obu grupach. Świadczy to o bardziej jednorodnym rynku mieszkań,
gdzie obecność ochrony nie prowadzi do ekstremalnie wysokich cen


#### Interpretacja wyników analizy wpływu obecności komórki lokatorskiej na cenę mieszkań: 

W analizie porównaliśmy ceny mieszkań w dwóch grupach: z komórką lokatorską i bez komórki lokatorskiej.
Obserwacje z wykresów pudełkowych wskazują, że obecność komórki lokatorskiej ma zauważalny, 
ale nie zawsze decydujący wpływ na ceny mieszkań, a różnice w medianach cen są zależne od lokalizacji.

W miastach takich jak Warszawa i Kraków, gdzie ceny mieszkań są najwyższe, 
posiadanie komórki lokatorskiej wiąże się z nieco wyższymi medianami cen.
Sugeruje to, że w tych dużych aglomeracjach dodatkowe przestrzenie, takie jak komórki lokatorskie,
są postrzegane jako istotne udogodnienie, które podnosi wartość nieruchomości.
W tych miastach wyższe ceny mogą wynikać z większego popytu na mieszkania, 
gdzie dodatkowa przestrzeń magazynowa jest uznawana za istotny atut.

W miastach takich jak Rzeszów czy Radom wpływ posiadania komórki lokatorskiej 
na ceny mieszkań jest minimalny. Różnice między medianami cen w obu grupach są nieznaczne, 
co wskazuje, że w tych lokalizacjach komórka lokatorska nie odgrywa kluczowej roli 
w kształtowaniu wartości nieruchomości. W tych miastach inne czynniki,
takie jak standard mieszkania czy jego lokalizacja, mogą mieć większe znaczenie dla ceny.

Z kolei w miastach takich jak Białystok różnice między medianami cen są również niewielkie,
co może sugerować, że komórka lokatorska nie jest postrzegana jako ważne udogodnienie,
a inne czynniki mogą mieć dominujący wpływ na ceny mieszkań. 

Obecność komórki lokatorskiej w mieszkaniach ma zauważalny wpływ na ceny, 
ale jego znaczenie różni się w zależności od miasta. W większych miastach, 
takich jak Warszawa i Kraków, komórka lokatorska przyczynia się do wyższych cen, 
natomiast w miastach średniej wielkości, takich jak Rzeszów czy Radom, jej wpływ na ceny jest minimalny.
...
korelacje_cena_odl <- dane_filtr %>%
  group_by(city) %>% 
  summarise(
    korelacja = cor(centre_distance, price) 
  )
