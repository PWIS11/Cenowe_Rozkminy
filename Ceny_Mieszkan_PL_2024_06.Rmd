...
Na podstawie przeprowadzonych testów Chi-kwadrat oraz wykresów możemy sformułować
następujące wnioski:
...
### Cel
Zdecydowaliśmy się na przeprowadzenie testu Ancova na podstawie zmiennych:
...
Test ANCOVA pozwoli zbadać, w jakim stopniu te czynniki oddziałują na ceny nieruchomości 
po uwzględnieniu efektu zmiennej `price_per_square_meter`, czyli ceny za metr kwadratowy.
Dzięki temu możliwe będzie kontrolowanie wpływu tej zmiennej oraz ocena, czy pozostałe 
cechy mają istotne znaczenie w modelowaniu cen mieszkań. Dodatkowo test umożliwi określenie,
czy wpływ analizowanych czynników jest statystycznie potwierdzony.

### Definicja Testu ANCOVA
Test ANCOVA  to statystyczna metoda łącząca analizę wariancji z analizą regresji. 
Pozwala na ocenę różnic między średnimi wartościami w różnych grupach, jednocześnie 
kontrolując wpływ jednej lub więcej zmiennych ciągłych (kowariantów). <br>

- **Hipoteza zerowa (H₀):** Po uwzględnieniu wpływu kowariantu nie ma istotnych
różnic między grupami.<br>
- **Hipoteza alternatywna (H₁):** Po uwzględnieniu wpływu kowariantu istnieją
istotne różnice między grupami.
...
{error}}}\) – średni kwadrat błędu,<br>
- \(\text{SS}_{\text{effect}}\) – suma kwadratów związana z efektem czynnika, <br>
...
Zmienna **`has_balcony`** ma również istotny wpływ na cenę, przy p = 0.0294. Wartość F wynosząca 4,803 sugeruje, że posiadanie balkonu wpływa na cenę nieruchomości, po uwzględnieniu ceny za metr kwadratowy. 

#### **Obecność windy (`has_elevator`)**:
Zmienna **`has_elevator`** także ma istotny wpływ na cenę (p = 0,0259), z wartością F wynoszącą 5,029. Posiadanie windy w nieruchomości w Białymstoku wpływa na cenę, po uwzględnieniu innych zmiennych. 

#### **Obecność parkingu (`has_parking`)**:
Zmienna **`has_parking`** **nie jest istotna statystycznie**, ponieważ wartość p wynosi 0,4013. Wartość F 0,707 sugeruje, że dostępność parkingu nie ma statystycznie istotnego wpływu na cenę nieruchomości w Białymstoku, po uwzględnieniu ceny za metr kwadratowy i innych zmiennych.

#### **Interakcje**:
- **`price_per_square_meter:has_balcony` (interakcja ceny za $m^2$ z obecnością balkonu)**: Wartość F wynosząca 0,597 i p = 0,4404 wskazują, że interakcja między ceną za $m^2$ a obecnością balkonu nie ma istotnego wpływu na cenę. P-wartość > 0,05 sugeruje, że efekt ceny za $m^2$ na cenę nieruchomości nie zmienia się w zależności od tego, czy mieszkanie ma balkon.
  
- **`price_per_square_meter:has_elevator` (interakcja ceny za $m^2$ z obecnością windy)**: Wartość F 0,547 i p = 0,4604 oznaczają, że interakcja między ceną za $m^2$ a obecnością windy także nie jest istotna. P-wartość > 0,05 sugeruje, że wpływ ceny za $m^2$ na cenę nie zależy od tego, czy mieszkanie ma windę.

- **`price_per_square_meter:has_parking` (interakcja ceny za $m^2$ z obecnością parkingu)**: Z kolei interakcja między ceną za $m^2$ a obecnością parkingu ma istotny wpływ na cenę (F = 11.364, p = 0.000883). P-wartość mniejsza niż 0,001 oznacza, że wpływ ceny za m² na cenę nieruchomości różni się w zależności od obecności parkingu. W praktyce oznacza to, że zmiana ceny za $m^2$ może mieć inny wpływ na cenę nieruchomości w zależności od tego, czy mieszkanie ma parking.


### 2. **Częstochowa:**

#### **Obecność balkonu (`has_balcony`)**:
Zmienna **`has_balcony`** również ma istotny wpływ na cenę (p = 0.01198). Wartość F wynosząca 6.374 sugeruje, że posiadanie balkonu wpływa na cenę nieruchomości, po uwzględnieniu ceny za metr kwadratowy. 

#### **Obecność parkingu (`has_parking`)**:
Zmienna **`has_parking`** również ma **istotny wpływ** na cenę (p = 0.00117), z wartością F 10.701. Posiadanie parkingu zwiększa cenę nieruchomości, co oznacza, że dostępność parkingu wpływa na wartość nieruchomości w Częstochowie. 

#### **Interakcje**:
- **`price_per_square_meter:has_balcony` (interakcja ceny za m² z obecnością balkonu)**: Wartość F wynosząca 1.344 i p = 0.2471 oznaczają, że interakcja między ceną za $m^2$ a obecnością balkonu nie ma istotnego wpływu na cenę. P-wartość > 0.05 sugeruje, że wpływ ceny za $m^2$ na cenę nieruchomości nie zmienia się w zależności od tego, czy mieszkanie ma balkon.
  
- **`price_per_square_meter:has_parking` (interakcja ceny za m² z obecnością parkingu)**: Wartość F 5.567 i p = 0.01881 wskazują, że interakcja między ceną za $m^2$ a obecnością parkingu jest istotna. P-wartość mniejsza niż 0,05 oznacza, że wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności od tego, czy mieszkanie ma parking. To sugeruje, że obecność parkingu może wzmacniać lub zmieniać efekt ceny za $m^2$ na cenę całkowitą.
...
#### **Obecność parkingu (`has_parking`)**:
Zmienna **`has_parking`** również ma istotny wpływ na cenę (p = 5.67e-09), z wartością F 37.252. Posiadanie parkingu zwiększa cenę nieruchomości, co oznacza, że dostępność parkingu ma bardzo silny wpływ na wartość nieruchomości w Radomiu. 

#### **Interakcje**:
- **`price_per_square_meter:has_parking` (interakcja ceny za $m^2$ z obecnością parkingu)**: Wartość F wynosząca 1.231 i p = 0.269 wskazują, że interakcja między ceną za $m^2$ a obecnością parkingu nie ma istotnego wpływu na cenę. P-wartość > 0,05 sugeruje, że efekt ceny za m² na cenę nieruchomości nie zmienia się w zależności od tego, czy mieszkanie ma parking.


### 4. **Kraków**:

#### **Obecność balkonu (`has_balcony`)**:
Zmienna **`has_balcony`** jest również bardzo istotna statystycznie (p < 2e-16). Wartość F wynosząca 90.922 sugeruje, że posiadanie balkonu ma bardzo silny wpływ na cenę nieruchomości. 

#### **Obecność windy (`has_elevator`)**:
Zmienna **`has_elevator`** ma istotny wpływ na cenę (p = 1.37e-05). Wartość F wynosząca 18.968 sugeruje, że posiadanie windy wpływa na cenę nieruchomości, po uwzględnieniu ceny za $m^2$. 

#### **Obecność parkingu (`has_parking`)**:
Zmienna **`has_parking`** również ma istotny wpływ na cenę (p = 3.60e-07), z wartością F 26.005. Posiadanie parkingu zwiększa cenę nieruchomości, co oznacza, że dostępność parkingu ma bardzo silny wpływ na wartość nieruchomości w Krakowie. 

#### **Obecność ochrony (`has_security`)**:
Zmienna **`has_security`** nie jest istotna statystycznie (p = 0.8549), co oznacza, że obecność ochrony nie ma wpływu na cenę nieruchomości w Krakowie.

#### **Obecność pomieszczenia gospodarczego (`has_storage_room`)**:
Zmienna **`has_storage_room`** ma marginalny wpływ na cenę (p = 0.0597), z wartością F 3.548. Chociaż p-wartość jest nieco większa niż 0.05, można uznać, że wpływ obecności pomieszczenia gospodarczego na cenę nieruchomości w Krakowie jest statystycznie marginalny, a efekt jest widoczny przy poziomie istotności 10%.

#### **Interakcje**:
- **`price_per_square_meter:has_balcony` (interakcja ceny za $m^2$ z obecnością balkonu)**: Wartość F wynosząca 0.351 i p = 0.5536 oznaczają, że interakcja między ceną za $m^2$ a obecnością balkonu nie ma istotnego wpływu na cenę.
  
- **`price_per_square_meter:has_elevator` (interakcja ceny za $m^2$ z obecnością windy)**: Wartość F wynosząca 6.220 i p = 0.0127 wskazują, że interakcja między ceną za $m^2$ a obecnością windy jest istotna statystycznie. P-wartość mniejsza niż 0.05 sugeruje, że wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności od tego, czy mieszkanie ma windę.

- **`price_per_square_meter:has_parking` (interakcja ceny za $m^2$ z obecnością parkingu)**: Wartość F wynosząca 0.097 i p = 0.7558 oznaczają, że interakcja między ceną za $m^2$ a obecnością parkingu nie ma istotnego wpływu na cenę.

- **`price_per_square_meter:has_security` (interakcja ceny za $m^2$ z obecnością ochrony)**: Wartość F wynosząca 2.619 i p = 0.1057 sugerują, że interakcja między ceną za $m^2$ a obecnością ochrony nie ma istotnego wpływu na cenę.

- **`price_per_square_meter:has_storage_room` (interakcja ceny za $m^2$ z obecnością pomieszczenia gospodarczego)**: Wartość F wynosząca 4.951 i p = 0.0261 wskazują, że interakcja między ceną za $m^2$ a obecnością pomieszczenia gospodarczego ma istotny wpływ na cenę nieruchomości w Krakowie. P-wartość mniejsza niż 0,05 sugeruje, że efekt ceny za $m^2$ jest różny w zależności od tego, czy mieszkanie ma pomieszczenie gospodarcze.


### 5. **Warszawa**:

#### **Obecność balkonu (`warszawa$has_balcony`)**:
Zmienna **`has_balcony`** jest bardzo istotna statystycznie (p < 2e-16). Wartość F wynosząca 129.058 sugeruje, że posiadanie balkonu ma bardzo silny wpływ na cenę nieruchomości. 

#### **Obecność windy (`warszawa$has_elevator`)**:
Zmienna **`has_elevator`** nie jest **istotna statystycznie** (p = 0.3099). Wartość F 1.031 sugeruje, że obecność windy nie ma wpływu na cenę nieruchomości w Warszawie.

#### **Obecność parkingu (`warszawa$has_parking`)**:
Zmienna **`has_parking`** jest **bardzo istotna statystycznie** (p < 2e-16), z wartością F wynoszącą 222.922. Posiadanie parkingu ma bardzo silny wpływ na cenę nieruchomości w Warszawie.

#### **Obecność ochrony (`warszawa$has_security`)**:
Zmienna **`has_security`** ma istotny wpływ na cenę (p = 9.17e-10), z wartością F wynoszącą 37.599. Posiadanie ochrony wpływa na cenę nieruchomości, co oznacza, że dostępność ochrony ma bardzo silny wpływ na wartość nieruchomości w Warszawie. 

#### **Obecność pomieszczenia gospodarczego (`warszawa$has_storage_room`)**:
Zmienna **`has_storage_room`** również ma istotny wpływ na cenę (p = 1.83e-08), z wartością F 31.739. Posiadanie pomieszczenia gospodarczego zwiększa cenę nieruchomości. 

#### **Interakcje**:
- **`price_per_square_meter:warszawa$has_balcony` (interakcja ceny za $m^2$ z obecnością balkonu)**: Wartość F wynosząca 5.165 i p = 0.0231 wskazują, że **interakcja między ceną za $m^2$ a obecnością balkonu jest istotna statystycznie**. P-wartość mniejsza niż 0,05 sugeruje, że wpływ ceny za m² na cenę nieruchomości różni się w zależności od tego, czy mieszkanie ma balkon.
  
- **`price_per_square_meter:warszawa$has_elevator` (interakcja ceny za $m^2$ z obecnością windy)**: Wartość F wynosząca 1.278 i p = 0.2584 sugerują, że interakcja między ceną za $m^2$ a obecnością windy nie ma istotnego wpływu na cenę.

- **`price_per_square_meter:warszawa$has_parking` (interakcja ceny za $m^2$ z obecnością parkingu)**: Wartość F wynosząca 17.210 i p = 3.39e-05 wskazują, że interakcja między ceną za $m^2$ a obecnością parkingu ma istotny wpływ na cenę. P-wartość mniejsza niż 0,001 sugeruje, że wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności od tego, czy mieszkanie ma parking.

- **`price_per_square_meter:warszawa$has_security` (interakcja ceny za $m^2$ z obecnością ochrony)**: Wartość F wynosząca 1.677 i p = 0.1954 sugerują, że interakcja między ceną za $m^2$ a obecnością ochrony nie ma istotnego wpływu na cenę.

- **`price_per_square_meter:warszawa$has_storage_room` (interakcja ceny za $m^2$ z obecnością pomieszczenia gospodarczego)**: Wartość F wynosząca 15.797 i p = 7.12e-05 wskazują, że interakcja między ceną za $m^2$ a obecnością pomieszczenia gospodarczego ma istotny wpływ na cenę nieruchomości. P-wartość mniejsza niż 0,001 sugeruje, że wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności od tego, czy mieszkanie ma pomieszczenie gospodarcze.
...
#### **Obecność windy (has_elevator)**:
Zmienna has_elevator nie ma istotnego wpływu na cenę nieruchomości (p = 0.63387). Wartość F 0.228 sugeruje, że posiadanie windy nie wpływa znacząco na cenę nieruchomości w Rzeszowie.

#### **Obecność pomieszczenia gospodarczego (has_storage_room)**:
Zmienna **has_storage_room** ma marginalny wpływ na cenę nieruchomości (p = 0.06127). Wartość F 3.553 sugeruje, że posiadanie pomieszczenia gospodarczego może mieć wpływ na cenę nieruchomości, ale jest na granicy istotności statystycznej.

#### **Interakcje**:
- **`price_per_square_meter:has_elevator (interakcja ceny za $m^2$ z obecnością windy)`**: Wartość F wynosząca 0.504 i p = 0.47880 oznaczają, że interakcja między ceną za $m^2$ a obecnością windy nie ma istotnego wpływu na cenę nieruchomości.

- **`price_per_square_meter:has_storage_room (interakcja ceny za $m^2$ z obecnością pomieszczenia gospodarczego)`**: Wartość F 0.002 i p = 0.96371 sugerują, że interakcja między ceną za $m^2$ a obecnością pomieszczenia gospodarczego nie ma istotnego wpływu na cenę nieruchomości.


### **Wnioski z analizy:**

#### 1. **Białystok**
- **Posiadanie balkonu** i **posiadanie windy** mają istotny wpływ na cenę nieruchomości, co sugerują wartości p < 0.05 i wartości F > 4. Posiadanie tych cech zwiększa cenę nieruchomości po uwzględnieniu innych czynników, takich jak cena za metr kwadratowy.
- **Obecność parkingu** nie ma statystycznie istotnego wpływu na cenę nieruchomości w Białymstoku, co sugeruje wysoka wartość p (0,4013).
- Interakcja **ceny za metr kwadratowy** z obecnością **parkingu** ma znaczący wpływ na cenę, co oznacza, że wpływ ceny za $m^2$ na cenę nieruchomości może się różnić w zależności od tego, czy mieszkanie ma parking.

#### 2. **Częstochowa**
- **Posiadanie balkonu** i **parkingu** mają istotny wpływ na cenę nieruchomości (p < 0.05). Zmienna **has_parking** ma silniejszy wpływ na cenę, z wartością F wynoszącą 10.701.
- Interakcja ceny za $m^2$ z obecnością parkingu również jest istotna (p < 0.05), co sugeruje, że obecność parkingu może zmieniać sposób, w jaki cena za metr kwadratowy wpływa na cenę nieruchomości.

#### 3. **Radom**
- **Obecność parkingu** ma bardzo silny wpływ na cenę nieruchomości (p = 5.67e-09, F = 37.252), co wskazuje na ogromne znaczenie parkingu w tym mieście.
- **Interakcja ceny za $m^2$ z obecnością parkingu** nie ma jednak istotnego wpływu na cenę nieruchomości, co może sugerować, że w Radomiu wpływ ceny za m² na cenę jest mniej wrażliwy na obecność parkingu.

#### 4. **Kraków**
- **Posiadanie balkonu**, **windy** i **parkingu** mają bardzo silny wpływ na cenę nieruchomości (p < 0.05). Wartość F dla każdej z tych zmiennych jest bardzo wysoka, co podkreśla ich wpływ na cenę.
- Interakcja ceny za m² z obecnością **windy** oraz **pomieszczenia gospodarczego** jest istotna statystycznie (p < 0.05). Oznacza to, że wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności od tego, czy mieszkanie ma windę lub pomieszczenie gospodarcze.
  
#### 5. **Warszawa**
- **Posiadanie balkonu**, **parkingu**, **ochrony** i **pomieszczenia gospodarczego** mają bardzo silny wpływ na cenę nieruchomości (p < 0.05). Wartość F dla zmiennej **has_parking** wynosząca 222.922 jest najwyższa, co wskazuje, że dostępność parkingu ma szczególnie duże znaczenie w Warszawie.
- Interakcje ceny za m² z obecnością **balkonu**, **parkingu** i **pomieszczenia gospodarczego** są istotne, co sugeruje, że zmiana ceny za $m^2$ może mieć różny wpływ na cenę nieruchomości w zależności od obecności tych cech.

#### 6. **Rzeszów**
- **Obecność windy** oraz **pomieszczenia gospodarczego** mają marginalny wpływ na cenę nieruchomości. Wartości p są bliskie granicy istotności, co sugeruje, że ich wpływ na cenę jest stosunkowo słaby, ale nie wykluczają one możliwości pewnego wpływu na cenę nieruchomości.
- Interakcje ceny za $m^2$ z obecnością **windy** oraz **pomieszczenia gospodarczego** nie mają istotnego wpływu na cenę nieruchomości (p > 0.05), co oznacza, że te cechy nie zmieniają zależności między ceną za $m^2$ a ceną nieruchomości w Rzeszowie.

### **Podsumowanie głównych wniosków**
- **Balkon i parking** są kluczowymi czynnikami wpływającymi na cenę nieruchomości w większości miast, choć ich wpływ może się różnić w zależności od lokalizacji. 
- **Windy** są istotne w niektórych miastach, takich jak Kraków, ale nie mają tak silnego wpływu wszędzie (np. w Rzeszowie).
- Interakcje między **ceną za metr kwadratowy** a obecnością **windy**, **parkingu** czy **balkonu** są istotne tylko w wybranych miastach, co sugeruje, że nie we wszystkich miastach te cechy wchodzą w interakcje z ceną za m² w sposób, który zmienia finalną cenę nieruchomości.
- **Obecność ochrony** i **pomieszczenia gospodarczego** wpływają na ceny w miastach takich jak Warszawa i Kraków, ale ich wpływ jest mniej wyraźny w innych miastach.


Na podstawie tych wyników, osoby inwestujące w nieruchomości w miastach takich jak Warszawa, Kraków i Częstochowa powinny szczególnie brać pod uwagę obecność parkingu, balkonu i windy, ponieważ te cechy mają najwięcej do powiedzenia w ustalaniu ceny nieruchomości. Z kolei w miastach takich jak Radom, Rzeszów czy Białystok, wpływ tych cech na cenę jest mniejszy, co może sugerować inne czynniki mogą mieć większe znaczenie przy wycenie nieruchomości.
