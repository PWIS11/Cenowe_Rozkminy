...
Test t-Studenta jest statystycznym testem hipotezy, który służy do porównania średnich wartości dwóch grup. Jest szeroko stosowany w sytuacjach, gdy dane mają rozkład normalny, a celem jest sprawdzenie, czy istnieje istotna różnica między średnimi tych grup.

#### Rodzaje testów t-Studenta

Test t-Studenta można podzielić na dwa główne typy:
1. **Test dla zmiennych zależnych**: Używany, gdy te same jednostki są mierzone w dwóch warunkach (np. przed i po interwencji).
2. **Test dla zmiennych niezależnych**: Używany, gdy porównuje się średnie dwóch niezależnych grup (np. grupy eksperymentalnej i kontrolnej).
...
- **Hipoteza alternatywna ($H_1$)**: Średnie w obu grupach nie są równe (w teście dwustronnym) lub jedna średnia jest większa/mniejsza od drugiej (w teście jednostronnym).
...
Gdzie:
- $\bar{X}_1$ i $\bar{X}_2$ to średnie grup,
- $s_1^2$ i $s_2^2$ to wariancje w grupach,
- $n_1$ i $n_2$ to liczności próbek.

#### Założenia testu t-Studenta

Aby poprawnie zastosować test t-Studenta, należy spełnić następujące założenia:
1. **Rozkład normalny**: Dane w obu grupach powinny pochodzić z rozkładu normalnego.
2. **Jednorodność wariancji**: Wariancje w obu grupach powinny być podobne (sprawdzane np. testem Levene’a).
3. **Niezależność obserwacji**: Dane w grupach powinny być od siebie niezależne.

#### Warianty testu

- **Test dwustronny**: Sprawdza, czy średnie w grupach są różne.
- **Test jednostronny**: Sprawdza, czy średnia jednej grupy jest większa/mniejsza od średniej drugiej grupy.

#### Interpretacja wyników

Wynik testu t-Studenta przedstawiany jest jako wartość statystyki $t$ oraz wartość p (p-value):
- Jeśli wartość p < $\alpha$ (np. 0.05), odrzucamy hipotezę zerową na rzecz hipotezy alternatywnej.
- Jeśli wartość p $\geq \alpha$, brak podstaw do odrzucenia hipotezy zerowej.

#### Zastosowania

Test t-Studenta znajduje zastosowanie w wielu dziedzinach, takich jak:
- Porównanie wyników testów przed i po szkoleniu,
- Analiza skuteczności terapii między grupą eksperymentalną a kontrolną,
- Porównanie cen nieruchomości w różnych lokalizacjach.

#DODAĆ tutaj test T-Studenta
...
wynagrodzenia <- data.frame(
  Miasto = c("Gdańsk", "Warszawa", "Kraków", "Częstochowa", "Rzeszów", "Białystok"),
  Wynagrodzenie = c(10017.25, 9673.42, 10173.41, 8380.28, 7187.74, 6806.05)
)
...
srednie_ceny <- dane %>%
  ...
  filter(city %in% c("Gdańsk", "Warszawa", "Kraków", "Częstochowa", "Rzeszów", "Białystok")) %>%
  rename(Miasto = city)
...
kolo_grad <- colorRamp(c("red", "green"))
...
1.  **Najlepsza dostępność mieszkań: Częstochowa**

    -   **Liczba m² za średnie wynagrodzenie**: Najwyższa spośród
        wszystkich miast (ponad 1 m²).

    -   **Cena za m²**: Najniższa (ok. 7,345 PLN), co w połączeniu z
        umiarkowanymi wynagrodzeniami (ok. 8,380 PLN) czyni mieszkania w
        Częstochowie najbardziej przystępnymi.

2.  **Najniższa dostępność mieszkań: Warszawa**

    -   **Liczba m² za średnie wynagrodzenie**: Najniższa (ok. 0.4 m²).

    -   **Cena za m²**: Najwyższa (ok. 15,695 PLN), co przy
        wynagrodzeniach na poziomie ok. 9,673 PLN skutkuje
        najtrudniejszym dostępem do mieszkań.

    -   Warszawa reprezentuje rynek o wysokich kosztach życia i
        mieszkania.

3.  **Średnia dostępność: Gdańsk i Kraków**

    -   **Liczba m² za średnie wynagrodzenie**: Na poziomie zbliżonym do
        siebie (ok. 0.6 m²).

    -   **Ceny za m²**: Wysokie (ok. 15,402 PLN w Gdańsku i 17,134 PLN w
        Krakowie).

    -   Wynagrodzenia w tych miastach są wysokie, co łagodzi wpływ
        drogich nieruchomości.

4.  **Regiony o niższych wynagrodzeniach: Rzeszów i Białystok**

    -   **Liczba m² za średnie wynagrodzenie**: Ograniczona (poniżej 0.5
        m²).

    -   **Cena za m²**: Niższa niż w największych miastach, ale
        wynagrodzenia w Rzeszowie (7,187 PLN) i Białymstoku (6,806 PLN)
        są zbyt niskie, by znacząco poprawić dostępność mieszkań.

#### **Wnioski z wizualizacji**

1.  **Dominacja Częstochowy**:

    -   Częstochowa wyróżnia się wyjątkowo korzystną kombinacją
        wynagrodzeń i cen mieszkań. Warto zwrócić uwagę na potencjalne
        możliwości inwestycyjne w tym mieście.

2.  **Konkurencja największych miast**:

    -   Gdańsk i Kraków, choć drogie, zapewniają umiarkowaną dostępność
        dzięki wysokim wynagrodzeniom. Warszawa pozostaje poza
        konkurencją jako najdroższe miasto.

3.  **Wyzwania w mniejszych miastach**:

    -   W Rzeszowie i Białymstoku sytuacja wymaga poprawy wynagrodzeń
        lub wsparcia w dostępności mieszkań, aby regiony te mogły
        konkurować z większymi ośrodkami.
...
-   **Najkrótszy czas zakupu (Częstochowa)**:
    ...
    -   Czas zakupu mieszkania przy minimalnej cenie to mniej niż 50
        miesięcy.
...
-   **Najdłuższy czas zakupu (Warszawa)**:
    ...
    -   Czas zakupu mieszkania w maksymalnej cenie wynosi ponad 300
        miesięcy (25 lat), co wskazuje na wysokie ceny mieszkań w
        stolicy.
...
-   **Stosunkowo stabilne wartości w Częstochowie i Białymstoku**:
    ...
    -   W obu miastach różnice między czasem dla minimalnej, średniej i
        maksymalnej ceny są najmniejsze. Sugeruje to bardziej jednorodne
        ceny mieszkań.

-   **Duże różnice w Warszawie, Gdańsku i Krakowie**:

    -   W miastach o wysokich cenach mieszkań różnice między czasem
        zakupu w zależności od przedziału cenowego są znaczące. Wskazuje
        to na dużą rozpiętość cenową na rynku nieruchomości.

**3. Trendy regionalne**

-   **Miasta północne i południowe**:

    -   Gdańsk i Kraków mają podobne trendy do Warszawy, choć ich czas
        zakupu w maksymalnych cenach jest nieco krótszy.

-   **Miasta mniejsze**:

    -   Rzeszów i Białystok oferują większą dostępność mieszkań, ale
        wciąż wymagają relatywnie długiego czasu na zakup w maksymalnych
        cenach (ok. 200 miesięcy).
