...
## Test Anova i test Tukeya

### Cel
Celem przeprowadzenia testu ANOVA oraz testu Tukeya jest szczegółowa analiza różnic w średnich cenach nieruchomości w wybranych miastach. Test ANOVA pozwala na ocenę, czy istnieje statystycznie istotna różnica między średnimi cenami nieruchomości w różnych miastach, co jest kluczowe dla zrozumienia, czy ceny w tych miastach są podobne, czy też występują znaczące różnice, które mogą wynikać z różnych czynników ekonomicznych, społecznych lub infrastrukturalnych.

### Definicja Testu ANOVA
Test ANOVA to statystyczna metoda służąca do analizy różnic między średnimi wartościami w różnych grupach. W szczególności:
- **Hipoteza zerowa (H₀):** Średnie we wszystkich grupach są równe.
...
gdzie:
- \(\text{MS}_{\text{between}} = \frac{\text{SS}_{\text{between}}}{df_{\text{between}}}\) – średni kwadrat między grupami,
- \(\text{MS}_{\text{within}} = \frac{\text{SS}_{\text{within}}}{df_{\text{within}}}\) – średni kwadrat wewnątrz grup,
- \(\text{SS}_{\text{between}}\) – suma kwadratów odchyleń między grupami,
- \(\text{SS}_{\text{within}}\) – suma kwadratów odchyleń wewnątrz grup,
- \(df_{\text{between}}\) i \(df_{\text{within}}\) – liczby stopni swobody odpowiednio dla efektu między grupami i resztowego.

### Definicja Testu Tukeya
Test Tukeya (Tukey's Honest Significant Difference, HSD) jest testem post hoc stosowanym po przeprowadzeniu analizy ANOVA, aby zidentyfikować, które grupy (np. miasta) różnią się między sobą pod względem średnich wartości. 

Test ten porównuje każdą możliwą parę grup i oblicza różnice między średnimi wartościami. Następnie ocenia, czy różnice te są statystycznie istotne, wykorzystując jednoczesne przedziały ufności i korektę dla wielokrotnych porównań. Dzięki temu test Tukeya minimalizuje ryzyko popełnienia błędu pierwszego rodzaju (fałszywe odrzucenie hipotezy zerowej).

Formuła statystyki testu Tukeya:

\[
q = \frac{\text{M}_{i} - \text{M}_{j}}{\sqrt{\frac{\text{MS}_{\text{within}}}{n}}}
\]

gdzie:
- \(\text{M}_{i}\) i \(\text{M}_{j}\) – średnie wartości dwóch porównywanych grup,
- \(\text{MS}_{\text{within}}\) – średni kwadrat wewnątrz grup (pochodzący z ANOVA),
- \(n\) – liczebność próby w każdej grupie.
...
```{r test_ANOVA,Porównanie_średnich_cen_nieruchomości_w_różnych_miastach, echo=FALSE}

set.seed(123)
średnie_ceny_nieruchomości_w_wybranych_miastach <- data.frame(
  Miasto = rep(c("Białystok", "Częstochowa", "Radom", "Kraków", "Warszawa", "Rzeszów"), each = 10),
  Cena = c(
    rnorm(10, mean = 521907.07, sd = 52190.7),    #(średnia powierzchnia wybranych miast x średnia cena za m2)
    rnorm(10, mean = 381839.72, sd = 38183.97),
    rnorm(10, mean = 384402.66, sd = 38440.27),
    rnorm(10, mean = 948791.53, sd = 94879.15),
    rnorm(10, mean = 1059721.79, sd = 105972.18),
    rnorm(10, mean = 648969.25, sd = 64896.93)
  )
)

średnie_ceny_nieruchomości_w_wybranych_miastach$Miasto <- as.factor(średnie_ceny_nieruchomości_w_wybranych_miastach$Miasto)

# Przeprowadzenie testu ANOVA
model <- aov(Cena ~ Miasto, data = średnie_ceny_nieruchomości_w_wybranych_miastach)
#summary(model)
...
```
Wyniki analizy ANOVA oraz testu Tukeya dotyczące średnich cen nieruchomości 
w wybranych miastach pozwalają na ocenę, czy różnice w cenach nieruchomości
pomiędzy miastami są statystycznie istotne.

### 1. **Wyniki testu ANOVA:**

Ponieważ wartość p jest mniejsza niż 0.05 (w tym przypadku jest to < 2e-16), 
możemy odrzucić hipotezę zerową. Oznacza to, że średnie ceny nieruchomości 
w różnych miastach różnią się w sposób statystycznie istotny. Innymi słowy,
przynajmniej jedno z badanych miast ma inną średnią cenę nieruchomości niż 
pozostałe.

Test ANOVA wskazuje na istnienie różnic, ale nie precyzuje, które konkretnie 
miasta różnią się między sobą. Aby dowiedzieć się, które pary miast są różne,
przeprowadzamy dodatkowy test post-hoc, na przykład test Tukeya.

### 2. **Wyniki testu Tukeya (Tukey HSD):**

Test Tukeya jest przeprowadzany, aby zidentyfikować, które pary miast różnią się 
od siebie pod względem średnich cen nieruchomości. Z wyników testu Tukeya możemy
wyciągnąć następujące wnioski:

- **Częstochowa vs Białystok**: Istnieje istotna różnica w średnich cenach 
nieruchomości pomiędzy Częstochową a Białymstokiem, przy czym średnia cena 
w Białymstoku jest wyższa o 135996.10 PLN. Wartość p wynosi 1.81e-04,
co wskazuje na bardzo silną istotność statystyczną.
  
- **Kraków vs Białystok**: Istnieje bardzo istotna różnica pomiędzy Krakowem
a Białymstokiem, a średnia cena w Krakowie jest wyższa o 453545.01 PLN.
Wartość p wynosi 4.37e-13, co oznacza, że różnica jest ekstremalnie statystycznie
istotna.

- **Radom vs Białystok**: Różnica pomiędzy Radomiem a Białymstokiem jest również 
istotna, a średnia cena w Radomiu jest niższa o 157719.33 PLN. Wartość p wynosi
1.21e-05, co wskazuje na istotność statystyczną.

- **Rzeszów vs Białystok**: Różnica w cenach pomiędzy Rzeszowem a Białymstokiem
jest również istotna, a średnia cena w Rzeszowie jest wyższa o 137554.16 PLN. 
Wartość p wynosi 1.50e-04, co wskazuje na istotność statystyczną.

- **Warszawa vs Białystok**: Różnica w cenach pomiędzy Warszawą a Białymstokiem 
jest bardzo istotna, a średnia cena w Warszawie jest wyższa o 532996.35 PLN.
Wartość p wynosi 4.37e-13, co oznacza bardzo silną istotność statystyczną.

W analizie dalszych par miast zauważamy różne wyniki:<br>

-  **Kraków vs Częstochowa**: Istnieje istotna różnica (średnia cena w Krakowie 
jest wyższa o 589541.11 PLN, p = 4.37e-13).<br>
-  **Radom vs Częstochowa**: Różnica w cenach nie jest istotna (p = 0.9719), 
co sugeruje, że ceny nieruchomości w tych dwóch miastach są podobne.<br>
-  **Rzeszów vs Częstochowa**: Istnieje istotna różnica (średnia cena 
w Rzeszowie jest wyższa o 273550.26 PLN, p = 3.98e-12).<br>
-  **Warszawa vs Częstochowa**: Istnieje bardzo istotna różnica (średnia cena 
w Warszawie jest wyższa o 668992.45 PLN, p = 4.37e-13).

Analizując pary miast:

- **Radom vs Kraków** oraz **Rzeszów vs Kraków**: Istnieją istotne różnice 
(p < 0.05), z większymi cenami w Krakowie.<br>
- **Warszawa vs Kraków**: Różnica nie jest istotna (p = 0.072), co sugeruje, 
że ceny w Warszawie i Krakowie mogą być porównywalne.<br>
- **Rzeszów vs Radom**: Istnieje istotna różnica na korzyść Rzeszowa (średnia 
cena wyższa o 295273.49 PLN, p = 7.14e-13).<br>
- **Warszawa vs Radom**: Istnieje bardzo istotna różnica (średnia cena wyższa 
o 690715.68 PLN, p = 4.37e-13).<br>
- **Warszawa vs Rzeszów**: Istnieje bardzo istotna różnica (średnia cena wyższa
o 395442.20 PLN, p = 4.38e-13).


```{r Wizualizacja_testów, echo=FALSE}
# Boxploty dla średnich cen nieruchomości w wybranych miastach 
ggplot(średnie_ceny_nieruchomości_w_wybranych_miastach, aes(x = Miasto, y = Cena)) +
  geom_boxplot(fill = "skyblue", color = "black") +  
  geom_hline(aes(yintercept = mean(Cena)), color = "red", linetype = "dashed") + 
  theme_minimal() +
  labs(title = "Porównanie średnich cen nieruchomości w wybranych miastach",
       x = "Miasto",
       y = "Cena") +
...
### Podsumowanie:
Z przeprowadzonego testu ANOVA i testu Tukeya wynika, że **średnie ceny 
nieruchomości w wybranych miastach różnią się znacząco**. Istnieje wiele par miast, 
które różnią się od siebie statystycznie istotnie, z największymi różnicami 
pomiędzy miastami takimi jak Warszawa, Kraków, Białystok i Częstochowa. 

Największe różnice występują pomiędzy Warszawą i Białymstokiem, Warszawą 
i Częstochową oraz Krakowem i Białymstokiem. <br>W przypadku miast takich jak Radom 
i Częstochowa różnice są mniej wyraźne.
...
levene_results <- data.frame(
  Variable = c("has_parking", "has_balcony", "has_elevator", "has_security", "has_storage_room"),
  F_Statistic = c(
...
levene_results <- levene_results %>%
  mutate(
    Interpretation = ifelse(p_value < 0.05, "Wariancje różnią się istotnie", "Wariancje są jednorodne")
  )
...
Celem naszego projektu było zrozumienie zmienności cen mieszkań w Polsce w oparciu o dane rynkowe zebrane w 2024 roku. Analiza miała na celu zidentyfikowanie głównych trendów, czynników wpływających na ceny mieszkań oraz zaproponowanie rekomendacji dla potencjalnych inwestorów i osób poszukujących mieszkań.
...
2. **Przygotowanie danych**: Dane zostały oczyszczone i znormalizowane, aby umożliwić ich właściwą analizę. Usunięto duplikaty oraz niekompletne rekordy.
3. **Analiza statystyczna**: Wykorzystaliśmy różne techniki analizy statystycznej, w tym:
   - Testy istotności, takie jak test t-Studenta i ANOVA, w celu porównania średnich cen mieszkań w różnych regionach.
   - Analizę korelacji, aby zidentyfikować zależności między zmiennymi, takimi jak powierzchnia mieszkania a cena za metr kwadratowy.
   - Testy normalności, takie jak test Shapiro-Wilka, aby sprawdzić rozkład danych.
...
1. **Główne trendy**: Zauważyliśmy, że największy wzrost cen mieszkań wystąpił w dużych miastach, takich jak Warszawa, Kraków i Gdańsk, podczas gdy mniejsze miasta i wsie wykazywały stabilność cen.
...
3. **Regionalne różnice**: Wschodnia Polska wykazywała znacząco niższe ceny w porównaniu z regionami zachodnimi.
...
# **Podsumowanie analizy cen mieszkań w wybranych miastach**  
