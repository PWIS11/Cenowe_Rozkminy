...
knitr::kable(tabela_typów_danych, caption = "Typy Danych oraz ich ilość")
```
...
knitr::kable(rodzaje_danych, caption = "Typy Danych")
...
```{r weryfikacja_logicznej_spójności_danych, echo=FALSE}
...
Dzięki zastosowaniu odpowiednich metod wizualizacji (np. wykresów pudełkowych i macierzy braków) oraz technik statystycznych (np. Z-score) możliwe było zarówno dokładne zrozumienie problemów związanych z danymi, jak i zaplanowanie działań naprawczych (???? nie wiem czy brzmi to "naprawczych" dobrze). Tak przygotowany zbiór danych stanowi podstawę do przeprowadzenia dalszych analiz i modelowania.  
...
dane$poiCount<-imputate_na(dane, poiCount, method = "median")
...
# po transformacji
dane$poiCount <- sqrt(dane$poiCount)
hist(na.omit(dane$poiCount), 
     probability = TRUE, 
     col = "goldenrod1", 
     main = "Histogram - poiCount", 
...
druga werjsa, którą wolimy ?

Zmienna **powierzchnia mieszkania w metrach kwadratowych** może wykazywać duże wartości w przypadku apartamentów luksusowych lub domów jednorodzinnych. Rozkład tej zmiennej naturalnie odzwierciedla różnorodność nieruchomości na rynku – od małych kawalerek po przestronne rezydencje. Podobnie, liczba pokoi może wahać się od jednego do kilkunastu w zależności od typu nieruchomości, co w pełni odpowiada rzeczywistości.

Kiedy analizowaliśmy dane o **piętrze mieszkania** i **liczbie pięter w budynku**, od razu dostrzegliśmy, że wyższe wartości (np. 10–30 pięter) są typowe dla wieżowców w dużych miastach. Transformacja tych zmiennych jest zatem zbędna. Podobnie w przypadku zmiennej opisującej **rok budowy**, każda wartość niesie istotne znaczenie – od historycznych kamienic, po nowoczesne apartamentowce – co czyni tę zmienną zrozumiałą w swojej pierwotnej formie. Zmienna opisująca **wiek budynku** naturalnie odzwierciedla różnice pomiędzy budynkami historycznymi a nowymi inwestycjami. Asymetria w rozkładzie tej zmiennej jest w pełni zrozumiała i nie wymaga dodatkowych przekształceń.

Z kolei zmienne odległościowe, takie jak **odległość od centrum miasta**, **szkoły**, czy **klinik**, również mają logiczne wytłumaczenie w zależności od lokalizacji nieruchomości. Mniejsze wartości oznaczają bardziej zurbanizowane tereny, podczas gdy większe odległości wskazują na przedmieścia.

Podsumowując, zrezygnowaliśmy z transformacji tych zmiennych, ponieważ ich obecny kształt pozwala na pełne zachowanie kontekstu analizy. Każda z nich odzwierciedla rzeczywistość rynku nieruchomości i umożliwia intuicyjne wnioski bez potrzeby dodatkowego przekształcania. Dzięki temu analiza wyników pozostaje przejrzysta, a dane zachowują swoją interpretowalność.
