...
```{r walidacja_i_korekta_wartości;floor_floorCount}


cor(dane$floorCount, dane$floor)  # Korelacja między floorCount a floor

dane$floorCount <- ifelse(dane$floor > dane$floorCount, dane$floor, dane$floorCount)

library(ggplot2)
ggplot(dane, aes(x = floorCount, y = floor)) +
  geom_point(color = "blue", alpha = 0.5) +
  labs(title = "Związek między liczbą kondygnacji a piętrem",
       x = "Liczba kondygnacji (floorCount)",
       y = "Piętro (floor)") +
  theme_minimal()
ggplot(dane, aes(x = floorCount, y = floor)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", col = "red") +
  ggtitle("Regresja między liczba kondygnacji a piętrem")
