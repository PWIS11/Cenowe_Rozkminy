...
-   **Bardzo duża liczba braków:** `condition` (74.0%) i
    `building_material` (40.9%). Ze względu na ich wysoką niekompletność
    zdecydowaliśmy się usunąć kolumnę condition, a dla building_material
    zastosujemy imputację najczęstszą wartością.

-   **Umiarkowana liczba braków:** `building_type` (20.5%), `floor`
    (16.6%), `build_year` (15.7%). Uzupełnimy brakujące wartości
    odpowiednio metodą najczęstszej wartości dla zmiennych
    kategorycznych (`building_type`) oraz medianą dla zmiennych
    liczbowych (`floor`, `build_year`).

-   **Niewielka liczba braków:** Kolumny takie jak `has_elevator`
    (4.46%) czy `college_distance` (2.72%) zostaną uzupełnione
    odpowiednio modą i medianą.
...
Zależności cenowe w wybranych miastach przedstawione zostały na
poniższym wykresie

```{r porównanie_miast, echo=FALSE, message = FALSE, fig.width=10, fig.height=6,}
# ggstatsplot::ggbetweenstats(
#   data = dane %>% filter(city %in% c("Warszawa", "Kraków", "Rzeszów", "Białystok", "Radom", "Częstochowa")),
#   x = city,
#   y = price_per_square_meter,
#   title = "Porównanie cen za m² w wybranych miastach"
# )
```

Wykres graficznie przedstawia wcześniej omówione różnice w cenach
mieszkań za metr kwadratowy w wybranych miastach. Wizualizacja
potwierdza podział na trzy grupy cenowe, ukazując wyraźne różnice między
najdroższymi, średnimi i najtańszymi lokalizacjami.

# **Analiza udogodnień w nieruchomościach**

Wykonamy analizę, aby sprawdzić, w jaki sposób różne udogodnienia, takie
jak balkon, winda, parking, ochrona czy komórka lokatorska, wpływają na
ceny mieszkań w wybranych miastach. Za pomocą wykresów pudełkowych
ocenimy, czy obecność tych udogodnień ma znaczący wpływ na wartość
nieruchomości oraz jak różnice te kształtują się w zależności od
lokalizacji.
...
# binary_vars <- c("has_parking", "has_balcony", "has_elevator", "has_security", "has_storage_room")
# 
# for (i in 1:(length(binary_vars) - 1)) {
#   for (j in (i + 1):length(binary_vars)) {
#     var1 <- sym(binary_vars[i])
#     var2 <- sym(binary_vars[j])
#     
#     plot <- ggbarstats(
#       data = dane_filtr,
#       x = !!var1,
#       y = !!var2,
#       title = paste("Zależność między", binary_vars[i], "a", binary_vars[j])
#     ) +
#       scale_fill_manual(values = c("orchid", "lightyellow")) +
#       theme_minimal()
#     
#     print(plot)
#   }
...
Na podstawie przeprowadzonych testów Chi-kwadrat oraz wykresów możemy
...
-   `has_parking` (p = 0.8754) -- nie ma podstaw do odrzucenia hipotezy
    zerowej o jednorodności wariancji, dlatego **wariancje są
    jednorodne**.
...
```{r MW tabela, echo=FALSE, escape=FALSE}
# Wczytanie pakietu gt
library(gt)
...
# Renderowanie tabeli w gt
summary_table %>%
  gt() %>%
  tab_header(title = "Podsumowanie wyników testów Manna-Whitneya dla miast i zmiennych kategorycznych.") %>%
  tab_style(
    style = list(
      cell_fill(color = "red"),
    ...
    locations = cells_body(
      columns = c(Interpretacja),  # Upewnienie się, że poprawnie wybieramy kolumnę
      rows = Interpretacja == "Istotna różnica"
    )
  )
```
...
-   `has_elevator`, `has_parking`, `has_balcony`: stwierdzono istotne
    różnice w rozkładach cen (p \< 0.05).
...
-   `has_elevator`, `has_parking`, `has_balcony`, `has_security`:
...
-   `has_elevator`, `has_balcony`, `has_security`, `has_storage_room`:
...
-   `has_elevator`, `has_security`, `has_storage_room`: brak istotnej
    różnicy (p ≥ 0.05).
...
-   `has_elevator`, `has_parking`, `has_balcony`: istotna różnica (p \<
    0.05).
...
zmiennych: `has_balcony`, `has_elevator`, `has_parking`, `has_security`
oraz `has_storage_room` ze względu na to, że w naszych poprzednich
...
`price_per_square_meter`, czyli ceny za metr kwadratowy. Dzięki temu
możliwe będzie kontrolowanie wpływu tej zmiennej oraz ocena, czy
pozostałe cechy mają istotne znaczenie w modelowaniu cen mieszkań.
...
Test ANCOVA to statystyczna metoda łącząca analizę wariancji z analizą
regresji. Pozwala na ocenę różnic między średnimi wartościami w różnych
grupach, jednocześnie kontrolując wpływ jednej lub więcej zmiennych
ciągłych (kowariantów). <br>

-   **Hipoteza zerowa (H₀):** Po uwzględnieniu wpływu kowariantu nie ma
    istotnych różnic między grupami.<br>
-   **Hipoteza alternatywna (H₁):** Po uwzględnieniu wpływu kowariantu
    istnieją istotne różnice między grupami.

<br> Statystyka testu **F** wyrażana jest wzorem:
...
gdzie: <br> -
$\text{MS}_{\text{effect}} = \frac{\text{SS}_{\text{effect}}}{df_{\text
{effect}}}$ – średni kwadrat efektu,<br> -
$\text{MS}_{\text{error}} = \frac{\text{SS}_{\text{error}}}{df_{\text
{error}}}$ – średni kwadrat błędu,<br> - $\text{SS}_{\text{effect}}$ –
suma kwadratów związana z efektem czynnika, <br> -
$\text{SS}_{\text{error}}$ – suma kwadratów błędu resztowego,<br> -
...
```{r test_ANOVA, echo=FALSE, warning=FALSE, message=FALSE}

#Zmienne do przeprowadzenia testu ANcOVA
...
# Test Ancovy dla Białegostoku: 

ancova_bialystok <- aov(price ~ price_per_square_meter + has_balcony + has_elevator + has_parking + 
                         price_per_square_meter:has_balcony + price_per_square_meter:has_elevator + price_per_square_meter:has_parking, 
                         data = bialystok)
#summary(ancova_bialystok)


# Test Ancovy dla Częstochowy: 
ancova_czestochowa <- aov(price ~ price_per_square_meter + has_balcony + has_parking + 
                           price_per_square_meter:has_balcony + price_per_square_meter:has_parking, 
                           data = czestochowa)
#summary(ancova_czestochowa)

# Test Ancovy dla Radomia: 
ancova_radom <- aov(price ~ price_per_square_meter + has_parking + 
                    price_per_square_meter:has_parking, 
                    data = radom)
#summary(ancova_radom)


# Test Anocvy dla Krakowa: 
ancova_krakow <- aov(price ~ price_per_square_meter + has_balcony + has_elevator + has_parking + has_security + has_storage_room + 
                      price_per_square_meter:has_balcony + price_per_square_meter:has_elevator + price_per_square_meter:has_parking + 
                      price_per_square_meter:has_security + price_per_square_meter:has_storage_room, 
                      data = krakow)
#summary(ancova_krakow)


# Test Anocvy dla Warszawy: 
ancova_warszawa <- aov(warszawa$price ~ warszawa$price_per_square_meter + warszawa$has_balcony + warszawa$has_elevator + warszawa$has_parking + warszawa$has_security + warszawa$has_storage_room +
                       warszawa$price_per_square_meter:warszawa$has_balcony + warszawa$price_per_square_meter:warszawa$has_elevator + 
                       warszawa$price_per_square_meter:warszawa$has_parking + warszawa$price_per_square_meter:warszawa$has_security + 
                       warszawa$price_per_square_meter:warszawa$has_storage_room, data = warszawa)
#summary(ancova_warszawa)


# Test Anocvy dla Rzeszowa: 
ancova_rzeszow <- aov(rzeszow$price ~ rzeszow$price_per_square_meter + rzeszow$has_elevator + rzeszow$has_storage_room + 
                      rzeszow$price_per_square_meter:rzeszow$has_elevator + rzeszow$price_per_square_meter:rzeszow$has_storage_room, 
                      data = rzeszow)
#summary(ancova_rzeszow)

```

### Wyniki testu

```{r wizualizacja_wyników_ANCOVY, echo=FALSE}
# Funkcja do dodania wyników do tabeli
...
summary_table <- data.frame(Miasto = character(), Zmienna = character(), 
                            F_value = numeric(), p_value = character(), stringsAsFactors = FALSE)

summary_table <- rbind(
  summary_table, add_to_table("Białystok", "has_balcony", 4.803, 0.0294),
  summary_table, add_to_table("Białystok", "has_elevator", 5.029, 0.0259),
  summary_table, add_to_table("Białystok", "has_parking", 0.707, 0.4013),
  summary_table, add_to_table("Białystok", "price_per_square_meter:has_balcony", 0.597, 0.4404),
  summary_table, add_to_table("Białystok", "price_per_square_meter:has_elevator", 0.547, 0.4604),
  summary_table, add_to_table("Białystok", "price_per_square_meter:has_parking", 11.364, 0.000883),
  
  summary_table, add_to_table("Częstochowa", "has_balcony", 6.374, 0.01198),
  summary_table, add_to_table("Częstochowa", "has_parking", 10.701, 0.00117),
  summary_table, add_to_table("Częstochowa", "price_per_square_meter:has_balcony", 1.344, 0.2471),
  summary_table, add_to_table("Częstochowa", "price_per_square_meter:has_parking", 5.567, 0.01881),
  
  summary_table, add_to_table("Radom", "has_parking", 37.252, 5.67e-09),
  summary_table, add_to_table("Radom", "price_per_square_meter:has_parking", 1.231, 0.269),
  
  summary_table, add_to_table("Kraków", "has_balcony", 90.922, "2e-16"),
  summary_table, add_to_table("Kraków", "has_elevator", 18.968, 1.37e-05),
  summary_table, add_to_table("Kraków", "has_parking", 26.005, 3.60e-07),
  summary_table, add_to_table("Kraków", "has_security", 0.032, 0.8549),
  summary_table, add_to_table("Kraków", "has_storage_room", 3.548, 0.0597),
  summary_table, add_to_table("Kraków", "price_per_square_meter:has_balcony", 0.351, 0.5536),
  summary_table, add_to_table("Kraków", "price_per_square_meter:has_elevator", 6.220, 0.0127),
  summary_table, add_to_table("Kraków", "price_per_square_meter:has_parking", 0.097, 0.7558),
  summary_table, add_to_table("Kraków", "price_per_square_meter:has_security", 2.619, 0.1057),
  summary_table, add_to_table("Kraków", "price_per_square_meter:has_storage_room", 4.951, 0.0261),
  
  summary_table, add_to_table("Warszawa", "has_balcony", 129.058, "2e-16"),
  summary_table, add_to_table("Warszawa", "has_elevator", 1.031, 0.3099),
  summary_table, add_to_table("Warszawa", "has_parking", 222.922, "2e-16"),
  summary_table, add_to_table("Warszawa", "has_security", 37.599, "9.17e-10"),
  summary_table, add_to_table("Warszawa", "has_storage_room", 31.739, "1.83e-08"),
  summary_table, add_to_table("Warszawa", "price_per_square_meter:has_balcony", 5.165, 0.0231),
  summary_table, add_to_table("Warszawa", "price_per_square_meter:has_elevator", 1.278, 0.2584),
  summary_table, add_to_table("Warszawa", "price_per_square_meter:has_parking", 17.210, "3.39e-05"),
  summary_table, add_to_table("Warszawa", "price_per_square_meter:has_security", 1.677, 0.1954),
  summary_table, add_to_table("Warszawa", "price_per_square_meter:has_storage_room", 15.797, "7.12e-05"),
  
  summary_table, add_to_table("Rzeszów", "has_elevator", 0.228, 0.63387),
  summary_table, add_to_table("Rzeszów", "has_storage_room", 3.553, 0.06127),
  summary_table, add_to_table("Rzeszów", "price_per_square_meter:has_elevator", 0.504, 0.47880),
  summary_table, add_to_table("Rzeszów", "price_per_square_meter:has_storage_room", 0.002, 0.96371)
)

library(kableExtra)
kable(summary_table, caption = "Podsumowanie wyników testów ANCOVA") %>%
...
#### **Obecność balkonu (`has_balcony`)**:

Zmienna **`has_balcony`** ma również istotny wpływ na cenę, przy p =
0.0294. Wartość F wynosząca 4,803 sugeruje, że posiadanie balkonu wpływa
na cenę nieruchomości, po uwzględnieniu ceny za metr kwadratowy.

#### **Obecność windy (`has_elevator`)**:

Zmienna **`has_elevator`** także ma istotny wpływ na cenę (p = 0,0259),
z wartością F wynoszącą 5,029. Posiadanie windy w nieruchomości w
Białymstoku wpływa na cenę, po uwzględnieniu innych zmiennych.

#### **Obecność parkingu (`has_parking`)**:

Zmienna **`has_parking`** **nie jest istotna statystycznie**, ponieważ
wartość p wynosi 0,4013. Wartość F 0,707 sugeruje, że dostępność
parkingu nie ma statystycznie istotnego wpływu na cenę nieruchomości w
Białymstoku, po uwzględnieniu ceny za metr kwadratowy i innych
zmiennych.

#### **Interakcje**:

-   **`price_per_square_meter:has_balcony` (interakcja ceny za** $m^2$ z
    obecnością balkonu): Wartość F wynosząca 0,597 i p = 0,4404
    wskazują, że interakcja między ceną za $m^2$ a obecnością balkonu
    nie ma istotnego wpływu na cenę. P-wartość \> 0,05 sugeruje, że
    efekt ceny za $m^2$ na cenę nieruchomości nie zmienia się w
    zależności od tego, czy mieszkanie ma balkon.

-   **`price_per_square_meter:has_elevator` (interakcja ceny za** $m^2$
    z obecnością windy): Wartość F 0,547 i p = 0,4604 oznaczają, że
    interakcja między ceną za $m^2$ a obecnością windy także nie jest
    istotna. P-wartość \> 0,05 sugeruje, że wpływ ceny za $m^2$ na cenę
    nie zależy od tego, czy mieszkanie ma windę.

-   **`price_per_square_meter:has_parking` (interakcja ceny za** $m^2$ z
    obecnością parkingu): Z kolei interakcja między ceną za $m^2$ a
    obecnością parkingu ma istotny wpływ na cenę (F = 11.364, p =
    0.000883). P-wartość mniejsza niż 0,001 oznacza, że wpływ ceny za m²
    na cenę nieruchomości różni się w zależności od obecności parkingu.
    W praktyce oznacza to, że zmiana ceny za $m^2$ może mieć inny wpływ
    na cenę nieruchomości w zależności od tego, czy mieszkanie ma
    parking.
...
Zmienna **`has_balcony`** również ma istotny wpływ na cenę (p =
0.01198). Wartość F wynosząca 6.374 sugeruje, że posiadanie balkonu
wpływa na cenę nieruchomości, po uwzględnieniu ceny za metr kwadratowy.

#### **Obecność parkingu (`has_parking`)**:

Zmienna **`has_parking`** również ma **istotny wpływ** na cenę (p =
0.00117), z wartością F 10.701. Posiadanie parkingu zwiększa cenę
nieruchomości, co oznacza, że dostępność parkingu wpływa na wartość
nieruchomości w Częstochowie.

#### **Interakcje**:

-   **`price_per_square_meter:has_balcony` (interakcja ceny za m² z
    obecnością balkonu)**: Wartość F wynosząca 1.344 i p = 0.2471
    oznaczają, że interakcja między ceną za $m^2$ a obecnością balkonu
    nie ma istotnego wpływu na cenę. P-wartość \> 0.05 sugeruje, że
    wpływ ceny za $m^2$ na cenę nieruchomości nie zmienia się w
    zależności od tego, czy mieszkanie ma balkon.

-   **`price_per_square_meter:has_parking` (interakcja ceny za m² z
    obecnością parkingu)**: Wartość F 5.567 i p = 0.01881 wskazują, że
    interakcja między ceną za $m^2$ a obecnością parkingu jest istotna.
    P-wartość mniejsza niż 0,05 oznacza, że wpływ ceny za $m^2$ na cenę
    nieruchomości różni się w zależności od tego, czy mieszkanie ma
    parking. To sugeruje, że obecność parkingu może wzmacniać lub
    zmieniać efekt ceny za $m^2$ na cenę całkowitą.
...
Zmienna **`has_parking`** również ma istotny wpływ na cenę (p =
5.67e-09), z wartością F 37.252. Posiadanie parkingu zwiększa cenę
nieruchomości, co oznacza, że dostępność parkingu ma bardzo silny wpływ
na wartość nieruchomości w Radomiu.

#### **Interakcje**:

-   **`price_per_square_meter:has_parking` (interakcja ceny za** $m^2$ z
    obecnością parkingu): Wartość F wynosząca 1.231 i p = 0.269
    wskazują, że interakcja między ceną za $m^2$ a obecnością parkingu
    nie ma istotnego wpływu na cenę. P-wartość \> 0,05 sugeruje, że
    efekt ceny za m² na cenę nieruchomości nie zmienia się w zależności
    od tego, czy mieszkanie ma parking.

### 4. **Kraków**:

#### **Obecność balkonu (`has_balcony`)**:

Zmienna **`has_balcony`** jest również bardzo istotna statystycznie (p
\< 2e-16). Wartość F wynosząca 90.922 sugeruje, że posiadanie balkonu ma
bardzo silny wpływ na cenę nieruchomości.

#### **Obecność windy (`has_elevator`)**:

Zmienna **`has_elevator`** ma istotny wpływ na cenę (p = 1.37e-05).
Wartość F wynosząca 18.968 sugeruje, że posiadanie windy wpływa na cenę
nieruchomości, po uwzględnieniu ceny za $m^2$.

#### **Obecność parkingu (`has_parking`)**:

Zmienna **`has_parking`** również ma istotny wpływ na cenę (p =
3.60e-07), z wartością F 26.005. Posiadanie parkingu zwiększa cenę
nieruchomości, co oznacza, że dostępność parkingu ma bardzo silny wpływ
na wartość nieruchomości w Krakowie.

#### **Obecność ochrony (`has_security`)**:

Zmienna **`has_security`** nie jest istotna statystycznie (p = 0.8549),
co oznacza, że obecność ochrony nie ma wpływu na cenę nieruchomości w
Krakowie.

#### **Obecność pomieszczenia gospodarczego (`has_storage_room`)**:

Zmienna **`has_storage_room`** ma marginalny wpływ na cenę (p = 0.0597),
z wartością F 3.548. Chociaż p-wartość jest nieco większa niż 0.05,
można uznać, że wpływ obecności pomieszczenia gospodarczego na cenę
nieruchomości w Krakowie jest statystycznie marginalny, a efekt jest
widoczny przy poziomie istotności 10%.

#### **Interakcje**:

-   **`price_per_square_meter:has_balcony` (interakcja ceny za** $m^2$ z
    obecnością balkonu): Wartość F wynosząca 0.351 i p = 0.5536
    oznaczają, że interakcja między ceną za $m^2$ a obecnością balkonu
    nie ma istotnego wpływu na cenę.

-   **`price_per_square_meter:has_elevator` (interakcja ceny za** $m^2$
    z obecnością windy): Wartość F wynosząca 6.220 i p = 0.0127
    wskazują, że interakcja między ceną za $m^2$ a obecnością windy jest
    istotna statystycznie. P-wartość mniejsza niż 0.05 sugeruje, że
    wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności od
    tego, czy mieszkanie ma windę.

-   **`price_per_square_meter:has_parking` (interakcja ceny za** $m^2$ z
    obecnością parkingu): Wartość F wynosząca 0.097 i p = 0.7558
    oznaczają, że interakcja między ceną za $m^2$ a obecnością parkingu
    nie ma istotnego wpływu na cenę.

-   **`price_per_square_meter:has_security` (interakcja ceny za** $m^2$
    z obecnością ochrony): Wartość F wynosząca 2.619 i p = 0.1057
    sugerują, że interakcja między ceną za $m^2$ a obecnością ochrony
    nie ma istotnego wpływu na cenę.

-   **`price_per_square_meter:has_storage_room` (interakcja ceny za**
    $m^2$ z obecnością pomieszczenia gospodarczego): Wartość F wynosząca
    4.951 i p = 0.0261 wskazują, że interakcja między ceną za $m^2$ a
    obecnością pomieszczenia gospodarczego ma istotny wpływ na cenę
    nieruchomości w Krakowie. P-wartość mniejsza niż 0,05 sugeruje, że
    efekt ceny za $m^2$ jest różny w zależności od tego, czy mieszkanie
    ma pomieszczenie gospodarcze.

### 5. **Warszawa**:

#### **Obecność balkonu (`warszawa$has_balcony`)**:

Zmienna **`has_balcony`** jest bardzo istotna statystycznie (p \<
2e-16). Wartość F wynosząca 129.058 sugeruje, że posiadanie balkonu ma
bardzo silny wpływ na cenę nieruchomości.

#### **Obecność windy (`warszawa$has_elevator`)**:

Zmienna **`has_elevator`** nie jest **istotna statystycznie** (p =
0.3099). Wartość F 1.031 sugeruje, że obecność windy nie ma wpływu na
cenę nieruchomości w Warszawie.

#### **Obecność parkingu (`warszawa$has_parking`)**:

Zmienna **`has_parking`** jest **bardzo istotna statystycznie** (p \<
2e-16), z wartością F wynoszącą 222.922. Posiadanie parkingu ma bardzo
silny wpływ na cenę nieruchomości w Warszawie.

#### **Obecność ochrony (`warszawa$has_security`)**:

Zmienna **`has_security`** ma istotny wpływ na cenę (p = 9.17e-10), z
wartością F wynoszącą 37.599. Posiadanie ochrony wpływa na cenę
nieruchomości, co oznacza, że dostępność ochrony ma bardzo silny wpływ
na wartość nieruchomości w Warszawie.

#### **Obecność pomieszczenia gospodarczego (`warszawa$has_storage_room`)**:

Zmienna **`has_storage_room`** również ma istotny wpływ na cenę (p =
1.83e-08), z wartością F 31.739. Posiadanie pomieszczenia gospodarczego
zwiększa cenę nieruchomości.

#### **Interakcje**:

-   **`price_per_square_meter:warszawa$has_balcony` (interakcja ceny
    za** $m^2$ z obecnością balkonu): Wartość F wynosząca 5.165 i p =
    0.0231 wskazują, że **interakcja między ceną za** $m^2$ a obecnością
    balkonu jest istotna statystycznie. P-wartość mniejsza niż 0,05
    sugeruje, że wpływ ceny za m² na cenę nieruchomości różni się w
    zależności od tego, czy mieszkanie ma balkon.

-   **`price_per_square_meter:warszawa$has_elevator` (interakcja ceny
    za** $m^2$ z obecnością windy): Wartość F wynosząca 1.278 i p =
    0.2584 sugerują, że interakcja między ceną za $m^2$ a obecnością
    windy nie ma istotnego wpływu na cenę.

-   **`price_per_square_meter:warszawa$has_parking` (interakcja ceny
    za** $m^2$ z obecnością parkingu): Wartość F wynosząca 17.210 i p =
    3.39e-05 wskazują, że interakcja między ceną za $m^2$ a obecnością
    parkingu ma istotny wpływ na cenę. P-wartość mniejsza niż 0,001
    sugeruje, że wpływ ceny za $m^2$ na cenę nieruchomości różni się w
    zależności od tego, czy mieszkanie ma parking.

-   **`price_per_square_meter:warszawa$has_security` (interakcja ceny
    za** $m^2$ z obecnością ochrony): Wartość F wynosząca 1.677 i p =
    0.1954 sugerują, że interakcja między ceną za $m^2$ a obecnością
    ochrony nie ma istotnego wpływu na cenę.

-   **`price_per_square_meter:warszawa$has_storage_room` (interakcja
    ceny za** $m^2$ z obecnością pomieszczenia gospodarczego): Wartość F
    wynosząca 15.797 i p = 7.12e-05 wskazują, że interakcja między ceną
    za $m^2$ a obecnością pomieszczenia gospodarczego ma istotny wpływ
    na cenę nieruchomości. P-wartość mniejsza niż 0,001 sugeruje, że
    wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności od
    tego, czy mieszkanie ma pomieszczenie gospodarcze.

### 6. **Rzeszów**:

#### **Obecność windy (has_elevator)**:

Zmienna has_elevator nie ma istotnego wpływu na cenę nieruchomości (p =
0.63387). Wartość F 0.228 sugeruje, że posiadanie windy nie wpływa
znacząco na cenę nieruchomości w Rzeszowie.

#### **Obecność pomieszczenia gospodarczego (has_storage_room)**:

Zmienna **has_storage_room** ma marginalny wpływ na cenę nieruchomości
(p = 0.06127). Wartość F 3.553 sugeruje, że posiadanie pomieszczenia
gospodarczego może mieć wpływ na cenę nieruchomości, ale jest na granicy
istotności statystycznej.

#### **Interakcje**:

-   **`price_per_square_meter:has_elevator (interakcja ceny za $m^2$ z obecnością windy)`**:
    Wartość F wynosząca 0.504 i p = 0.47880 oznaczają, że interakcja
    między ceną za $m^2$ a obecnością windy nie ma istotnego wpływu na
    cenę nieruchomości.

-   **`price_per_square_meter:has_storage_room (interakcja ceny za $m^2$ z obecnością pomieszczenia gospodarczego)`**:
    Wartość F 0.002 i p = 0.96371 sugerują, że interakcja między ceną za
    $m^2$ a obecnością pomieszczenia gospodarczego nie ma istotnego
...
-   **Posiadanie balkonu** i **posiadanie windy** mają istotny wpływ na
    cenę nieruchomości, co sugerują wartości p \< 0.05 i wartości F
    \> 4. Posiadanie tych cech zwiększa cenę nieruchomości po
    uwzględnieniu innych czynników, takich jak cena za metr kwadratowy.
-   **Obecność parkingu** nie ma statystycznie istotnego wpływu na cenę
    nieruchomości w Białymstoku, co sugeruje wysoka wartość p (0,4013).
-   Interakcja **ceny za metr kwadratowy** z obecnością **parkingu** ma
    znaczący wpływ na cenę, co oznacza, że wpływ ceny za $m^2$ na cenę
    nieruchomości może się różnić w zależności od tego, czy mieszkanie
    ma parking.

#### 2. **Częstochowa**

-   **Posiadanie balkonu** i **parkingu** mają istotny wpływ na cenę
    nieruchomości (p \< 0.05). Zmienna **has_parking** ma silniejszy
    wpływ na cenę, z wartością F wynoszącą 10.701.
-   Interakcja ceny za $m^2$ z obecnością parkingu również jest istotna
    (p \< 0.05), co sugeruje, że obecność parkingu może zmieniać sposób,
    w jaki cena za metr kwadratowy wpływa na cenę nieruchomości.

#### 3. **Radom**

-   **Obecność parkingu** ma bardzo silny wpływ na cenę nieruchomości (p
    = 5.67e-09, F = 37.252), co wskazuje na ogromne znaczenie parkingu w
    tym mieście.
-   **Interakcja ceny za** $m^2$ z obecnością parkingu nie ma jednak
    istotnego wpływu na cenę nieruchomości, co może sugerować, że w
    Radomiu wpływ ceny za m² na cenę jest mniej wrażliwy na obecność
    parkingu.

#### 4. **Kraków**

-   **Posiadanie balkonu**, **windy** i **parkingu** mają bardzo silny
    wpływ na cenę nieruchomości (p \< 0.05). Wartość F dla każdej z tych
    zmiennych jest bardzo wysoka, co podkreśla ich wpływ na cenę.
-   Interakcja ceny za m² z obecnością **windy** oraz **pomieszczenia
    gospodarczego** jest istotna statystycznie (p \< 0.05). Oznacza to,
    że wpływ ceny za $m^2$ na cenę nieruchomości różni się w zależności
    od tego, czy mieszkanie ma windę lub pomieszczenie gospodarcze.

#### 5. **Warszawa**

-   **Posiadanie balkonu**, **parkingu**, **ochrony** i **pomieszczenia
    gospodarczego** mają bardzo silny wpływ na cenę nieruchomości (p \<
    0.05). Wartość F dla zmiennej **has_parking** wynosząca 222.922 jest
    najwyższa, co wskazuje, że dostępność parkingu ma szczególnie duże
    znaczenie w Warszawie.
-   Interakcje ceny za m² z obecnością **balkonu**, **parkingu** i
    **pomieszczenia gospodarczego** są istotne, co sugeruje, że zmiana
    ceny za $m^2$ może mieć różny wpływ na cenę nieruchomości w
    zależności od obecności tych cech.

#### 6. **Rzeszów**

-   **Obecność windy** oraz **pomieszczenia gospodarczego** mają
    marginalny wpływ na cenę nieruchomości. Wartości p są bliskie
    granicy istotności, co sugeruje, że ich wpływ na cenę jest
    stosunkowo słaby, ale nie wykluczają one możliwości pewnego wpływu
    na cenę nieruchomości.
-   Interakcje ceny za $m^2$ z obecnością **windy** oraz **pomieszczenia
    gospodarczego** nie mają istotnego wpływu na cenę nieruchomości (p
    \> 0.05), co oznacza, że te cechy nie zmieniają zależności między
    ceną za $m^2$ a ceną nieruchomości w Rzeszowie.

### **Podsumowanie głównych wniosków**

-   **Balkon i parking** są kluczowymi czynnikami wpływającymi na cenę
    nieruchomości w większości miast, choć ich wpływ może się różnić w
    zależności od lokalizacji.
-   **Windy** są istotne w niektórych miastach, takich jak Kraków, ale
    nie mają tak silnego wpływu wszędzie (np. w Rzeszowie).
-   Interakcje między **ceną za metr kwadratowy** a obecnością
    **windy**, **parkingu** czy **balkonu** są istotne tylko w wybranych
    miastach, co sugeruje, że nie we wszystkich miastach te cechy
    wchodzą w interakcje z ceną za m² w sposób, który zmienia finalną
    cenę nieruchomości.
-   **Obecność ochrony** i **pomieszczenia gospodarczego** wpływają na
    ceny w miastach takich jak Warszawa i Kraków, ale ich wpływ jest
    mniej wyraźny w innych miastach.

Na podstawie tych wyników, osoby inwestujące w nieruchomości w miastach
takich jak Warszawa, Kraków i Częstochowa powinny szczególnie brać pod
uwagę obecność parkingu, balkonu i windy, ponieważ te cechy mają
najwięcej do powiedzenia w ustalaniu ceny nieruchomości. Z kolei w
miastach takich jak Radom, Rzeszów czy Białystok, wpływ tych cech na
cenę jest mniejszy, co może sugerować inne czynniki mogą mieć większe
znaczenie przy wycenie nieruchomości.

### **Krótka wersja podsumowania wyników ANCOVA**

+-----------+----------------------------+----------------------------+
| *         | **Główne wnioski**         | **Interakcje**             |
| *Miasto** |                            |                            |
+===========+============================+============================+
| **Bi      | \- Balkon (p=0.0294) i     | \- Interakcja cena/m² ×    |
| ałystok** | winda (p=0.0259) znacząco  | parking (p=0.000883) jest  |
|           | podnoszą cenę \            | istotna – wpływ ceny/m² na |
|           | - Parking (p=0.4013) nie   | cenę zależy od obecności   |
|           | ma istotnego wpływu        | parkingu                   |
+-----------+----------------------------+----------------------------+
| **Częs    | \- Balkon (p=0.01198) i    | \- Interakcja cena/m² ×    |
| tochowa** | parking (p=0.00117)        | parking (p=0.01881) jest   |
|           | istotnie podnoszą cenę     | istotna                    |
+-----------+----------------------------+----------------------------+
| **Radom** | \- Parking (p=5.67e-09) ma | \- Interakcja cena/m² ×    |
|           | bardzo silny wpływ na cenę | parking (p=0.269)          |
|           |                            | nieistotna                 |
+-----------+----------------------------+----------------------------+
| *         | \- Balkon (p\<2e-16),      | \- Istotne interakcje z    |
| *Kraków** | winda (p=1.37e-05) i       | windą (p=0.0127) i         |
|           | parking (p=3.60e-07) mocno | pomieszczeniem gosp.       |
|           | wpływają na cenę \         | (p=0.0261) – wpływ ceny/m² |
|           | - Ochrona (p=0.8549) nie   | różni się w zależności od  |
|           | ma wpływu \                | obecności tych udogodnień  |
|           | - Pomieszczenie            |                            |
|           | gospodarcze (p=0.0597) –   |                            |
|           | efekt graniczny            |                            |
+-----------+----------------------------+----------------------------+
| **W       | \- Balkon (p\<2e-16),      | \- Istotne interakcje z    |
| arszawa** | parking (p\<2e-16),        | balkonem (p=0.0231),       |
|           | ochrona (p=9.17e-10) i     | parkingiem (p=3.39e-05) i  |
|           | pomieszczenie gospodarcze  | pomieszczeniem gosp.       |
|           | (p=1.83e-08) wyraźnie      | (p=7.12e-05)               |
|           | zwiększają cenę \          |                            |
|           | - Winda (p=0.3099) nie     |                            |
|           | jest istotna               |                            |
+-----------+----------------------------+----------------------------+
| **        | \- Winda (p=0.63387) nie   | \- Brak istotnych          |
| Rzeszów** | ma wpływu \                | interakcji                 |
|           | - Pomieszczenie            |                            |
|           | gospodarcze (p=0.06127) –  |                            |
|           | marginalne znaczenie       |                            |
+-----------+----------------------------+----------------------------+

### Główne obserwacje:

-   **Balkon i parking** często najmocniej podwyższają cenę (np.
    Warszawa, Kraków, Radom, Częstochowa, Białystok).

-   **Winda** ma znaczenie w niektórych miastach (Kraków, Białystok),
    ale bywa nieistotna w innych (Warszawa, Rzeszów).

-   **Ochrona i pomieszczenie gospodarcze** wyraźnie działają zwłaszcza
    w Warszawie i Krakowie; gdzie indziej są mniej ważne lub nieistotne.

-   **Interakcje** (czyli wpływ danej cechy w zależności od ceny za m²)
    są czasem istotne, głównie w przypadku **parkingu** (Białystok,
    Częstochowa, Warszawa), a także **balkonu** (Warszawa) oraz **windy
    i pomieszczenia gosp.** (Kraków).

W praktyce oznacza to, że znaczenie konkretnych udogodnień (balkon,
parking, winda, ochrona, pomieszczenie gospodarcze) różni się w
zależności od miasta. Inwestorzy powinni więc zwrócić szczególną uwagę
na te cechy, które w danej lokalizacji najmocniej wpływają na wycenę
nieruchomości.
...
Celem projektu było zrozumienie, jakie czynniki wpływają na ceny
mieszkań w sześciu wybranych polskich miastach: Warszawie, Krakowie,
Rzeszowie, Białymstoku, Radomiu i Częstochowie. Przeprowadzona analiza
pozwoliła odpowiedzieć na kluczowe pytania badawcze oraz wyciągnąć
szczegółowe wnioski.
...
1.  **Zbieranie danych**: Przeanalizowaliśmy dostępne dane rynkowe
    obejmujące ceny mieszkań w różnych lokalizacjach Polski. Dane
    pochodziły z wiarygodnych źródeł, takich jak raporty rynkowe i bazy
    nieruchomości.
2.  **Przygotowanie danych**: Dane zostały oczyszczone i znormalizowane,
    aby umożliwić ich właściwą analizę. Dane poddane zostały imputacji,
    zaktualizowano nazwy kolumn czy zmiennych.
3.  **Analiza statystyczna**: Wykorzystaliśmy różne techniki analizy
    statystycznej, w tym:
    -   Testy istotności, takie jak ANOVA, w celu porównania średnich
        cen mieszkań w różnych regionach, test Chi-kwadrat czy Levene'a.
    -   Analizę korelacji, aby zidentyfikować zależności między
        zmiennymi, takimi jak powierzchnia mieszkania a cena za metr
        kwadratowy czy odległość do POI.
    -   Testy normalności, takie jak test Andersona-Darlinga, aby
        sprawdzić rozkład danych.
4.  **Wizualizacja danych**: Wyniki zostały przedstawione za pomocą
    interaktywnych wykresów i map, aby ułatwić zrozumienie wyników.
5.  **Wnioski i rekomendacje**: Opracowaliśmy wnioski, które mogą być
    użyteczne dla różnych grup interesariuszy, takich jak kupujący,
    deweloperzy i inwestorzy.

## Wyniki

1.  **Czynniki wpływające na ceny**:
    -   **Lokalizacja**: Lokalizacja nieruchomości okazała się kluczowym
        czynnikiem wpływającym na cenę. Mieszkania położone w centrum
        miast były wyceniane znacznie wyżej niż te na obrzeżach. Dostęp
        do infrastruktury, takich jak szkoły, sklepy i transport
        publiczny, dodatkowo zwiększał wartość nieruchomości.
    -   **Powierzchnia**: Cena za metr kwadratowy malała wraz ze
        wzrostem powierzchni mieszkania, co sugeruje, że większe
        mieszkania oferują lepszy stosunek ceny do metrażu.
    -   **Rok budowy**: Nowe mieszkania, zwłaszcza te zbudowane po 2015
        roku, były wyceniane wyżej ze względu na nowoczesne technologie
        budowlane, lepszą efektywność energetyczną i atrakcyjne projekty
        architektoniczne.
    -   **Stan techniczny**: Stan mieszkania, w tym jakość wykończenia
        oraz ewentualne potrzeby remontowe, istotnie wpływały na cenę.
        Mieszkania w stanie "do wprowadzenia" osiągały wyższe ceny niż
        te wymagające generalnego remontu.
    -   **Czynniki makroekonomiczne**: Wzrost stóp procentowych oraz
        inflacja miały pośredni wpływ na poziom cen, ograniczając
        zdolności nabywcze części klientów.
    -   **Udogodnienia i odległość od infrastruktury**: Bliskość do
        szkół, sklepów, parków czy transportu publicznego miała istotny
        wpływ na ceny mieszkań. Mieszkania zlokalizowane w pobliżu tych
        udogodnień były wyceniane wyżej.
2.  **Regionalne różnice**: Wschodnia Polska wykazywała znacząco niższe
    ceny w porównaniu z regionami zachodnimi.

## Wnioski

-   Dla inwestorów rekomendujemy koncentrację na rozwijających się
    miastach o wysokim potencjale wzrostu, takich jak Wrocław i Rzeszów.
-   Dla osób poszukujących mieszkań, mniejsze miasta oferują bardziej
    przystępne ceny przy zachowaniu stosunkowo dobrej infrastruktury.

## Refleksje

Projekt pozwolił nam na rozwinięcie umiejętności analizy danych,
interpretacji wyników oraz pracy zespołowej. W szczególności praca w
**RStudio** umożliwiła sprawne przetwarzanie danych, ich analizę i
wizualizację, co znacząco przyczyniło się do realizacji naszych celów.
...
1.  **Od czego zależy cena mieszkań?**\
    ...
    -   **Cech mieszkania**, takich jak liczba pokoi, powierzchnia, rok
        budowy oraz obecność udogodnień (balkon, winda, parking).\
    -   **Charakterystyki okolicy**, m.in. liczby punktów użyteczności
        publicznej (POI) w pobliżu, takich jak szkoły, restauracje czy
        kliniki.

2.  **Jakie różnice w cenach występują pomiędzy miastami?**\
    Miasta zostały podzielone na trzy grupy cenowe:

    -   **Najdroższe:** Warszawa (18 503 PLN/m²) i Kraków (17 135
        PLN/m²).\
    -   **Średnie:** Rzeszów (10 803 PLN/m²) i Białystok (10 089
        PLN/m²).\
    -   **Najtańsze:** Radom (6 929 PLN/m²) i Częstochowa (7 034
        PLN/m²).\
        Różnice w cenach wynikają z poziomu urbanizacji, rozwoju
        gospodarczego i dostępności infrastruktury w każdym mieście.

3.  **Czy odległość od centrum ma znaczenie?**\
    Odległość od centrum miasta istotnie wpływa na ceny mieszkań.
    Najbardziej zauważalne jest to w Warszawie i Krakowie, gdzie
    mieszkania w centralnych lokalizacjach osiągają najwyższe ceny. W
    mniejszych miastach, takich jak Radom czy Częstochowa, ten wpływ
    jest mniej wyraźny.
...
4.  **Które cechy mieszkań są najbardziej cenione?**\
    ...
    -   Powierzchnia mieszkania oraz liczba pokoi, co przekłada się na
...
2.  **Odległość od centrum**
    -   W dużych miastach, takich jak Warszawa i Kraków, ceny spadają
        wraz z odległością od centrum. W mniejszych miastach, takich jak
        Radom i Częstochowa, wpływ ten jest mniej zauważalny.
3.  **Rola udogodnień**
    -   Mieszkania z balkonem, miejscem parkingowym lub windą osiągają
        ceny wyższe niż te bez takich udogodnień, zwłaszcza w dużych
        aglomeracjach.\
    -   W miastach takich jak Warszawa i Kraków kluczowym czynnikiem
        wpływającym na wartość nieruchomości są również dodatkowe usługi
        w budynkach, np. ochrona czy komórki lokatorskie.
4.  **Powierzchnia i liczba pokoi**
    -   Typowe mieszkania w analizowanych miastach mają 2-3 pokoje i
        powierzchnię 50–60 m².
5.  **Wpływ demografii i wynagrodzeń**
    -   W miastach o wyższych średnich wynagrodzeniach (np. Warszawa,
        Kraków) ceny mieszkań są proporcjonalnie wyższe, co wskazuje na
        związek między lokalnym rynkiem pracy a wartością
        nieruchomości.\
    -   Regiony o niższych dochodach, takie jak Radom czy Częstochowa,
        oferują tańsze nieruchomości, ale mają również mniejszy
        potencjał inwestycyjny.
6.  **Obserwacje dotyczące rynku**
    -   Rynek w Warszawie i Krakowie charakteryzuje się dużą zmiennością
        cen, podczas gdy w Radomiu i Częstochowie jest bardziej
        stabilny.\
    -   W miastach średnich, takich jak Rzeszów i Białystok, zauważalny
        jest trend wzrostowy, co wskazuje na rosnący potencjał
        inwestycyjny.
...
1.  **Warszawa:** Najwyższe ceny wynikają z dużego zapotrzebowania na
    mieszkania w prestiżowych lokalizacjach, rozwiniętej infrastruktury
    i szerokiego dostępu do usług.\
2.  **Kraków:** Drugie najdroższe miasto, gdzie ceny są determinowane
    bliskością atrakcji turystycznych oraz rozwiniętym rynkiem wynajmu.\
3.  **Rzeszów:** Miasto w średnim przedziale cenowym, które rozwija się
    jako lokalne centrum biznesowe.\
4.  **Białystok:** Charakteryzuje się umiarkowanymi cenami, wynikającymi
    z mniejszego zapotrzebowania i niższego poziomu urbanizacji.\
5.  **Radom:** Najtańsze miast, zdominowane przez starsze budynki i
    ograniczone możliwości inwestycyjne.\
6.  **Częstochowa:** Drugie najtańsze miasto w badaniu, gdzie ceny są
    najniższe z uwagi na mniejszą atrakcyjność inwestycyjną i
    ograniczoną infrastrukturę.

------------------------------------------------------------------------

### **Wnioski końcowe**

Przeprowadzona analiza wykazała, że ceny mieszkań w dużym stopniu zależą
od lokalizacji, charakterystyki mieszkania oraz dostępności usług w
okolicy. Miasta takie jak Warszawa i Kraków dominują pod względem cen,
co wynika z ich centralnej roli w gospodarce. Z kolei mniejsze miasta,
takie jak Radom i Częstochowa, oferują tańsze mieszkania, ale z
ograniczonym potencjałem inwestycyjnym.

```{r}
library(leaflet)
library(dplyr)

# Wybranie top 5 najdroższych i top 5 najtańszych mieszkań dla każdego miasta
dane_top <- dane_filtr %>%
  group_by(city) %>%
  arrange(desc(price)) %>%
  slice_head(n = 5) %>%
...
# Tworzenie mapy z filtrowanymi danymi
leaflet(dane_top) %>%
  addTiles() %>%
  addCircleMarkers(
    ~longitude, ~latitude,
    radius = ~sqrt(price) / 200,
    color = ~ifelse(price == max(price, na.rm = TRUE), "orange", "tomato"),
    popup = ~paste0(
      "<b>Cena:</b> ", price, " PLN<br>",
      "<b>Miasto:</b> ", city, "<br>",
      "<b>Odległość od centrum:</b> ", centre_distance, " km<br>",
      "<b>Liczba pokoi:</b> ", rooms, "<br>",
      "<b>Winda:</b> ", ifelse(has_elevator == "yes", "Tak", "Nie"), "<br>",
      "<b>Balkon:</b> ", ifelse(has_balcony == "yes", "Tak", "Nie"), "<br>"
    ),
    fillOpacity = 0.7
  ) %>%
  addLegend(
    position = "topright",
    colors = c("tomato", "orange"),
    labels = c("Top 5 najtańszych", "Top 5 najdroższych"),
    title = "Legenda cen"
  )
