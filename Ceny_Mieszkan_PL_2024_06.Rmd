...
# Dodanie wyników testów ANCOVA do tabeli
summary_table <- rbind(
  summary_table, add_to_table("Białystok", "price_per_square_meter", 15.736, 9.78e-05),
  summary_table, add_to_table("Białystok", "has_balcony", 4.609, 0.0329),
  summary_table, add_to_table("Białystok", "has_elevator", 4.826, 0.0291),
  summary_table, add_to_table("Białystok", "has_parking", 0.679, 0.4109),
  
  summary_table, add_to_table("Częstochowa", "price_per_square_meter", 173.018, 2e-16),
  summary_table, add_to_table("Częstochowa", "has_balcony", 6.293, 0.01253),
  summary_table, add_to_table("Częstochowa", "has_parking", 10.564, 0.00126),
  
  summary_table, add_to_table("Radom", "price_per_square_meter", 42.43, 6.27e-10),
  summary_table, add_to_table("Radom", "has_parking", 37.21, 5.74e-09),
  
  summary_table, add_to_table("Kraków", "price_per_square_meter", 768.644, 2e-16),
  summary_table, add_to_table("Kraków", "has_balcony", 90.664, 2e-16),
  summary_table, add_to_table("Kraków", "has_elevator", 18.914, 1.41e-05),
  summary_table, add_to_table("Kraków", "has_parking", 25.931, 3.74e-07),
  summary_table, add_to_table("Kraków", "has_security", 0.033, 0.8551),
  summary_table, add_to_table("Kraków", "has_storage_room", 3.538, 0.0601),
  
  summary_table, add_to_table("Warszawa", "price_per_square_meter", 1566.379, 2e-16),
  summary_table, add_to_table("Warszawa", "has_balcony", 128.391, 2e-16),
  summary_table, add_to_table("Warszawa", "has_elevator", 1.026, 0.311),
  summary_table, add_to_table("Warszawa", "has_parking", 221.770, 2e-16),
  summary_table, add_to_table("Warszawa", "has_security", 37.404, 1.01e-09),
  summary_table, add_to_table("Warszawa", "has_storage_room", 31.575, 1.99e-08),
  
  summary_table, add_to_table("Rzeszów", "price_per_sqaure_meter", 10.928, 0.00117),
  summary_table, add_to_table("Rzeszów", "has_elevator", 0.230, 0.63226),
  summary_table, add_to_table("Rzeszów", "has_storage_room", 3.587, 0.06004)
)
...
### 1. **Białystok**:
Zmienna **`price_per_square_meter`** jest statystycznie istotna na poziomie 0,01% 
(p = 9.78e-05).Wartość F wynosząca 15,736 sugeruje, że cena za metr kwadratowy 
ma znaczący wpływ na cenę nieruchomości w Białymstoku, nawet po uwzględnieniu 
innych zmiennych.Trzy gwiazdki (***) wskazują na bardzo wysoką istotność 
statystyczną (p < 0,001), co oznacza, że istnieje 99,9% pewności, że efekt ceny 
za metr kwadratowy jest prawdziwy, a nie przypadkowy.

Zmienna **`has_balcony`** ma również istotny wpływ na cenę, przy wartościach p = 0,0329.
Wartość F wynosząca 4,609 sugeruje, że posiadanie balkonu wpływa na cenę nieruchomości, 
po uwzględnieniu ceny za metr kwadratowy. Jedna gwiazdka (*) wskazuje na 
umiarkowaną istotność statystyczną (p < 0,05), co oznacza, że istnieje 95% pewności,
że efekt posiadania balkonu na cenę nie jest przypadkowy.

Zmienna **`has_elevator`** także ma istotny wpływ na cenę (p = 0,0291), 
z wartością F 4,826. Posiadanie windy w nieruchomości w Białymstoku również wpływa 
na cenę, po uwzględnieniu innych zmiennych. Jedna gwiazdka (*) w wynikach oznacza 
istotność statystyczną na poziomie 5%, dając 95% pewności, że efekt obecności 
windy w nieruchomości nie jest przypadkowy.

Zmienna **`has_parking`** nie jest istotna statystycznie, ponieważ wartość p wynosi 0,4109.
Wartość F 0,679 sugeruje, że dostępność parkingu nie ma statystycznie istotnego 
wpływu na cenę nieruchomości w Białymstoku, po uwzględnieniu ceny za metr 
kwadratowy i innych zmiennych. Brak gwiazdek wskazuje, że wpływ posiadania 
parkingu na cenę nie jest statystycznie istotny.

### 2. **Częstochowa**:

Zmienna **`has_balcony`** jest istotna na poziomie 5% (p = 0,01253). 
Wartość F wynosząca 6,293 sugeruje, że posiadanie balkonu znacząco wpływa 
na cenę nieruchomości, po uwzględnieniu efektu **`price_per_square_meter`**. 
Wynik ten w tabeli podsumowania jest oznaczony jedną gwiazdką (*), co wskazuje 
na umiarkowaną istotność statystyczną (p < 0,05). Oznacza to, że istnieje 95% 
pewności, że efekt posiadania balkonu nie jest przypadkowy.

Podobnie, **`has_parking`** jest również statystycznie istotne, z wartością p 0,00126 
(znacznie mniejszą niż 0,05). Wartość F 10,564 sugeruje, że posiadanie parkingu
ma znaczący wpływ na ceny nieruchomości, nawet po uwzględnieniu **`price_per_square_meter`**.
W tabeli wynik ten jest oznaczony dwiema gwiazdkami (**), co oznacza wysoką 
istotność statystyczną (p < 0,01), wskazującą, że istnieje 99% pewności, iż efekt 
posiadania parkingu nie jest przypadkowy.

Posiadanie balkonu (**`has_balcony`**) jest istotnym czynnikiem wpływającym na cenę, 
przy czym wartość p wynosi 0,01253. Oznacza to, że nieruchomości z balkonem mają 
statystycznie istotną różnicę w cenie w porównaniu do nieruchomości bez balkonu, 
po uwzględnieniu ceny za metr kwadratowy. Jedna gwiazdka (*) widoczna w tabeli
wyników dodatkowo podkreśla, że różnica ta jest istotna na poziomie 5%.

Dostępność miejsc parkingowych (**`has_parking`**) jest również istotnym czynnikiem, 
przy czym wartość p wynosi 0,00126. Oznacza to, że dostępność miejsc parkingowych 
ma istotny wpływ na cenę, nawet po uwzględnieniu ceny za metr kwadratowy.
Dwie gwiazdki (**) przy tej zmiennej w wynikach analizy wskazują na bardzo wysoką 
istotność statystyczną (p < 0,01), co potwierdza, że wpływ miejsc parkingowych
na cenę jest statystycznie bardzo wiarygodny.


### 3. **Radom**:

Zmienna **`price_per_square_meter`** jest statystycznie istotna na poziomie 0,01% 
(p = 6.27e-10).Wartość F wynosząca 42,43 sugeruje, że cena za metr kwadratowy 
ma bardzo silny wpływ na cenę nieruchomości w Radomiu, nawet po uwzględnieniu 
innych zmiennych. Trzy gwiazdki (***) oznaczają bardzo wysoką istotność statystyczną 
(p < 0,001), co oznacza, że istnieje 99,9% pewności, że efekt ceny za metr 
kwadratowy jest prawdziwy, a nie przypadkowy.

Zmienna **`has_parking`** również jest statystycznie istotna na poziomie 0,01% (p = 5.74e-09).
Wartość F wynosząca 37,21 sugeruje, że dostępność parkingu ma bardzo silny wpływ 
na ceny nieruchomości w Radomiu, nawet po uwzględnieniu ceny za metr kwadratowy. 
Dwie gwiazdki (**) wskazują na bardzo wysoką istotność statystyczną (p < 0,01), 
co oznacza, że istnieje 99% pewności, że efekt posiadania parkingu nie jest przypadkowy.


### 4. **Kraków**:

Zmienna **`price_per_square_meter`** jest statystycznie istotna na poziomie 
0,01% (p < 2e-16).Wartość F wynosząca 768,644 sugeruje, że cena za metr kwadratowy 
ma bardzo silny wpływ na cenę nieruchomości w Krakowie. Trzy gwiazdki (***) wskazują 
na bardzo wysoką istotność statystyczną (p < 0,001), co oznacza, że istnieje 99,9% pewności, 
że efekt ceny za metr kwadratowy jest prawdziwy, a nie przypadkowy.

Zmienna **`has_balcony`** jest również statystycznie istotna na poziomie 0,01% 
(p < 2e-16). Wartość F wynosząca 90,664 sugeruje, że posiadanie balkonu ma silny 
wpływ na cenę nieruchomości w Krakowie. Trzy gwiazdki ( ***) oznaczają bardzo 
wysoką istotność statystyczną (p < 0,001), co oznacza, że istnieje 99,9% pewności, 
że efekt posiadania balkonu na cenę jest prawdziwy, a nie przypadkowy.

Zmienna **`has_elevator`** ma również statystycznie istotny wpływ na cenę, 
z wartością F wynoszącą 18,914 i p = 1.41e-05. Posiadanie windy w nieruchomości 
w Krakowie ma zauważalny wpływ na cenę. Trzy gwiazdki ( ***) wskazują na wysoką
istotność statystyczną (p < 0,001), co oznacza, że efekt posiadania windy 
jest prawdziwy, a nie przypadkowy.

Zmienna **`has_parking`** jest statystycznie istotna na poziomie 0,01% (p = 3.74e-07).  
Wartość F wynosząca 25,931 sugeruje, że dostępność parkingu również ma silny 
wpływ na cenę nieruchomości w Krakowie. Trzy gwiazdki ( ***) wskazują na bardzo
wysoką istotność statystyczną (p < 0,001), co oznacza, że efekt dostępności 
parkingu jest prawdziwy, a nie przypadkowy.

Zmienna **`has_security`** nie jest istotna statystycznie, ponieważ 
wartość p wynosi 0,8551. Oznacza to, że posiadanie ochrony w nieruchomości 
nie wpływa na jej cenę w sposób statystycznie istotny w Krakowie.

Zmienna **`has_storage_room`** jest bliska poziomowi istotności na poziomie 
10% (p = 0,0601). Wartość F wynosząca 3,538 sugeruje, że dostępność pomieszczenia 
do przechowywania ma marginalny wpływ na cenę nieruchomości w Krakowie. 
Jedna kropka (.) wskazuje na graniczną istotność statystyczną na poziomie 10% (p < 0,1).


### 5. **Warszawa**:

Zmienna **`price_per_square_meter`** jest statystycznie istotna na poziomie 
0,01% (p < 2e-16). Wartość F wynosząca 1566,379 sugeruje, że cena za metr 
kwadratowy ma bardzo silny wpływ na cenę nieruchomości w Warszawie. Trzy gwiazdki 
( ***) wskazują na bardzo wysoką istotność statystyczną (p < 0,001), co oznacza, 
że istnieje 99,9% pewności, że efekt ceny za metr kwadratowy jest prawdziwy,
a nie przypadkowy.

Zmienna **`has_balcony`** jest również statystycznie istotna na poziomie 0,01% (p < 2e-16).  
Wartość F wynosząca 128,391 sugeruje, że posiadanie balkonu ma bardzo silny wpływ 
na cenę nieruchomości w Warszawie. Trzy gwiazdki ( ***) oznaczają bardzo wysoką 
istotność statystyczną (p < 0,001), co oznacza, że istnieje 99,9% pewności, 
że efekt posiadania balkonu na cenę jest prawdziwy, a nie przypadkowy.

Zmienna **`has_elevator`** nie jest statystycznie istotna, ponieważ 
wartość p wynosi 0,311. Oznacza to, że posiadanie windy w nieruchomości w Warszawie 
nie wpływa na jej cenę w sposób statystycznie istotny.

Zmienna **`has_parking`** jest statystycznie istotna na poziomie 0,01% (p < 2e-16).  
Wartość F wynosząca 221,770 sugeruje, że dostępność parkingu ma bardzo silny wpływ 
na ceny nieruchomości w Warszawie. Trzy gwiazdki ( ***) wskazują na bardzo wysoką 
istotność statystyczną (p < 0,001), co oznacza, że efekt dostępności parkingu 
jest prawdziwy, a nie przypadkowy.

Zmienna **`has_security`** jest statystycznie istotna na poziomie 0,01% (p = 1.01e-09).  
Wartość F wynosząca 37,404 sugeruje, że posiadanie ochrony w nieruchomości
w Warszawie ma bardzo silny wpływ na jej cenę. Trzy gwiazdki ( ***) wskazują na 
bardzo wysoką istotność statystyczną (p < 0,001), co oznacza, że efekt posiadania 
ochrony na cenę nieruchomości jest prawdziwy, a nie przypadkowy.

Zmienna **`has_storage_room`** jest statystycznie istotna na poziomie 0,01% (p = 1.99e-08).  
Wartość F wynosząca 31,575 sugeruje, że dostępność pomieszczenia do przechowywania 
ma silny wpływ na cenę nieruchomości w Warszawie. Trzy gwiazdki ( ***) wskazują 
na bardzo wysoką istotność statystyczną (p < 0,001), co oznacza, że efekt
dostępności pomieszczenia do przechowywania na cenę nieruchomości jest prawdziwy, 
a nie przypadkowy.
...
Zmienna **`price_per_square_meter`** jest statystycznie istotna na poziomie
0,1% (p = 0.00117). Wartość F wynosząca 10,928 sugeruje, że cena za metr kwadratowy
ma istotny wpływ na cenę nieruchomości w Rzeszowie. Dwie gwiazdki (**) wskazują 
na wysoką istotność statystyczną (p < 0,01), co oznacza, że istnieje 99% pewności, 
że efekt ceny za metr kwadratowy jest prawdziwy, a nie przypadkowy.

Zmienna **`has_elevator`** nie jest statystycznie istotna, ponieważ
wartość p wynosi 0,63226. Oznacza to, że posiadanie windy w nieruchomości w Rzeszowie
nie wpływa na jej cenę w sposób statystycznie istotny.

Zmienna **`has_storage_room`** jest bliska poziomowi istotności 
na poziomie 10% (p = 0,06004).Wartość F wynosząca 3,587 sugeruje, że dostępność 
pomieszczenia do przechowywania ma marginalny wpływ na cenę nieruchomości w Rzeszowie. 
Jedna kropka (.) wskazuje na graniczną istotność statystyczną na poziomie 10% (p < 0,1).


### **Wnioski z analizy:**

**1. Wpływ ceny za metr kwadratowy (**`price_per_square_meter`**):**

- We wszystkich badanych miastach zmienna **`price_per_square_meter`** miała silny,
statystycznie istotny wpływ na cenę nieruchomości. Wartości p były bardzo niskie 
(zwykle < 0,001), co sugeruje, że efekt ceny za metr kwadratowy jest bardzo wiarygodny.
- Wartości F w różnych miastach wskazują na istotność tego efektu:
np. w Warszawie wartość F wynosi 1566,379, co oznacza bardzo silny wpływ tej zmiennej na cenę.

**2. Wpływ balkonu na cenę (**`has_balcony`**):**

- Zmienna **`has_balcony`** miała statystycznie istotny wpływ na cenę nieruchomości 
w większości miast. W **Białymstoku**, **Częstochowie**, **Krakowie**, **Warszawie** 
i **Radomiu** posiadanie balkonu było związane ze wzrostem ceny nieruchomości,
a wartości p były niskie, co oznacza wysoką istotność.** W Krakowie, Warszawie 
i Częstochowie efekt posiadania balkonu był szczególnie silny, co sugerują ich
wartości F.  W Białymstoku posiadanie balkonu miało bardziej umiarkowany wpływ, 
ale nadal statystycznie istotny.


**3. Wpływ parkingu na cenę (**`has_parking`**):**

- Zmienna **`has_parking`** miała również silny wpływ na ceny nieruchomości 
w wielu miastach. **Zmienna ta była istotna w Białymstoku, Częstochowie, Radomiu, 
Krakowie i Warszawie**. Wartości p były niskie (w większości przypadków < 0,01), 
co oznacza bardzo wysoką pewność, że efekt posiadania parkingu jest prawdziwy.
**Najsilniejszy wpływ parkingu na cenę nieruchomości odnotowano w Warszawie 
(F = 221,770), co sugeruje bardzo silny związek tej cechy z ceną.**

**4. Wpływ windy na cenę (**`has_elevator`**):**

- Wartości F wskazują, że **`has_elevator`** miało istotny wpływ na cenę
nieruchomości tylko w niektórych miastach. **Największy wpływ odnotowano 
w Krakowie (F = 18,914) oraz w Białymstoku (F = 4,826), gdzie zmienna ta 
była statystycznie istotna.** W innych miastach, jak Warszawa i Rzeszów, 
posiadanie windy nie miało istotnego wpływu na cenę (p > 0,05).

**5. Wpływ pomieszczenia gospodarczego na cenę (**`has_storage_room`**):**

- Wpływ  pomieszczenia gospodarczego **`has_storage_room`** miała marginalny wpływ na 
cenę nieruchomości w niektórych miastach, zwłaszcza w **Rzeszowie** i **Krakowie**, 
gdzie występowała graniczna istotność na poziomie 10% (p ≈ 0,06).
W mieście  **Warszawa** zmienna ta miała bardzo silny wpływ,
gdzie p wynosiło 1,99e-08, co sugeruje istotny związek z ceną nieruchomości.


**6. Wpływ ochrony na cenę (**`has_security`**):**

- Zmienna **`has_security`** nie była statystycznie istotna w **Białymstoku**, **Krakowie**
i **Rzeszowie**, co oznacza, że posiadanie ochrony nie miało wpływu na cenę nieruchomości
w tych miastach. W **Warszawie** natomiast zmienna ta była istotna, wskazując na wpływ ochrony na cenę.
