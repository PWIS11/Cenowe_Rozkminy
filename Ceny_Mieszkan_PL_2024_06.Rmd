...
# Imputacja medianą brakujących danych dla:

# floor
median_floor <- median(dane$floor, na.rm = TRUE)
dane$floor[is.na(dane$floor)] <- median_floor

# buildYear
median_buildYear <- median(dane$buildYear, na.rm = TRUE)
dane$buildYear[is.na(dane$buildYear)] <- median_buildYear

# hasElevator 
median_hasElevator <- median(dane$hasElevator, na.rm = TRUE)
dane$hasElevator[is.na(dane$hasElevator)] <- median_hasElevator

# collegeDistance 
median_collegeDistance <- median(dane$collegeDistance, na.rm = TRUE)
dane$collegeDistance[is.na(dane$collegeDistance)] <- median_collegeDistance


# Imputacja brakujących danych na najczęstszą wartość w:

# buildingMaterial
mode_buildingMaterial <- names(sort(table(dane$buildingMaterial), decreasing = TRUE))[1]
dane$buildingMaterial[is.na(dane$buildingMaterial)] <- mode_buildingMaterial

# type
mode_type <- names(sort(table(dane$type), decreasing = TRUE))[1]
dane$type[is.na(dane$type)] <- mode_type


# Dla wszystkich pozostałych kolumn, które mają mniej niż 1% NA 
...
ploti8 <- boxplot(dane$latitude, main = "Outliers in Latitude", col = "cadetblue3")
ploti9 <- boxplot(dane$longitude, main = "Outliers in Longitude", col = "pink1")
...
cat("Mediana dla price: ", median_price, "\n")
cat("Średnia dla price: ", mean_price, "\n")

# Mediana i średnia dla SquareMeters
median_squareMeters <- median(dane$squareMeters)
mean_squareMeters <- mean(dane$squareMeters)
cat("Mediana dla squareMeters: ", median_squareMeters, "\n")
cat("Średnia dla squareMeters: ", mean_squareMeters, "\n")
...
cat("Mediana dla pricePerSquareMeter: ", median_pricePerSquareMeter, "\n")
cat("Średnia dla pricePerSquareMeter: ", mean_pricePerSquareMeter, "\n")

# Mediana i średnia dla BuildingAge
median_buildingAge <- median(dane$buildingAge)
mean_buildingAge <- mean(dane$buildingAge)
cat("Mediana dla buildingAge: ", median_buildingAge, "\n")
cat("Średnia dla buildingAge: ", mean_buildingAge, "\n")

# Mediana i średnia dla Latitude
median_latitude <- median(dane$latitude)
mean_latitude <- mean(dane$latitude)
cat("Mediana dla Latitude: ", median_latitude, "\n")
cat("Średnia dla Latitude: ", mean_latitude, "\n")

# Mediana i średnia dla Longitude
median_longitude <- median(dane$longitude)
mean_longitude <- mean(dane$longitude)
cat("Mediana dla Longitude: ", median_longitude, "\n")
cat("Średnia dla Longitude: ", mean_longitude, "\n")


```
Do analizy obserwacji odstających posłużono się testem Grubbsa. W teście stwierdzono, że: 

1) dla squareMaters wartość 150 jest potencjalnie odstającym wynikiem. Jednakże z p-value wynoszącym 0.06864, co jest większe niż 0.05, uznano, że nie ma wystarczających dowodów, aby tę wartość odrzucić. Mediana dla squareMeters wynosi 52.81, a średnia 56.97. Obie te wartości są bliskie sobie, co wskazuje, że dane są rozłożone stosunkowo równomiernie, a różnice między nimi nie są duże. To sugeruje, że wartość 150 nie jest skrajnie różna od pozostałych danych. Wartość ta, choć wyraźnie wyższa, mieści się w szerszym kontekście rozkładu, gdyż mediana i średnia wskazują na brak silnych odchyleń w danych. 

2) dla price wartość 3 000 000 jest wartością odstającą. Zgonie z wynikiem testu Grubbsa, statystyka G wynosi 5.04755, p-value (0.004775) jest poniżej progu 0.05, co oznacza, że wynik jest statystycznie istotny. Należy ją traktować jako rzadki, ale możliwy przypadek bardzo drogiej nieruchomości. Boxplot pokazuje, że ta wartość nie jest błędem, lecz wynikiem specyficznej sytuacji na rynku.

3) dla pricePerSquareMeter Test Grubbsa stwierdził, że najwyższa wartość 30 702.75 jest potencjalną wartością odstającą. Wynik p-value (1) sugeruje, że nie powinniśmy odrzucać hipotezy zerowej, przez co nie uznano tej wartości za wartość odstającą i mieści się ona w ogólnym rozkładzie. 
