...
```{r walidacja_kontrolna_danych, echo=FALSE, out.width="83%", fig.width=14, fig.height=9}
...
validation_results <- data.frame(
  rule = rules_names,  # Nazwy reguł walidacji
  examined_observations = rep(21501, 8),  # Liczba obserwacji (dla każdej reguły ta sama wartość)
  observations_passing_rule = passes,  # Liczba obserwacji, które spełniają regułę
  observations_failing_rule = fails   # Liczba obserwacji, które nie spełniają regułę
)

# Nowy wykres z ulepszonym stylem
failing_plot <- ggplot(validation_results, aes(x = reorder(rule, -observations_failing_rule), y = observations_failing_rule)) +
  geom_bar(stat = "identity", fill = "#FF6F61", color = "black", size = 0.5, width = 0.7) +  # Zmieniony kolor i obramowanie
  geom_text(aes(label = observations_failing_rule), hjust = 1.1, size = 5, fontface = "bold", color = "white") +  # Białe liczby z pogrubieniem
  coord_flip() +  # Obrócenie wykresu, aby reguły były na osi Y
  labs(
    title = "Liczba niespełnionych reguł walidacji",
    x = "Reguły walidacji",
    y = "Liczba niespełnionych"
  ) +
  theme_minimal() +  # Minimalistyczny wygląd wykresu
  theme(
    plot.title = element_text(hjust = 0.5, size = 16, face = "bold", color = "#333333"),  # Tytuł pogrubiony
    axis.text.y = element_text(size = 10, color = "#333333", face = "italic"),  # Tekst osi Y w kursywie
    axis.title = element_text(size = 12, color = "#333333"),  # Tekst osi w ciemnym kolorze
    panel.grid = element_blank(),  # Usuwanie siatki
    plot.background = element_rect(fill = "white", color = "white")  # Tło wykresu
  )
