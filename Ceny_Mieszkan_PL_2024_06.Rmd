...
```{r test_ANCOVA, echo=FALSE, warning=FALSE, message=FALSE}
# Zmienne do przeprowadzenia testu ANCOVA:
zmienne_do_testu_Ancova <- data.frame(
...
ancova_bialystok <- aov(price ~ price_per_square_meter + has_balcony + +has_elevator + has_parking, data = bialystok)
#summary(ancova_bialystok)

# Test Ancovy dla Częstochowy: 
ancova_czestochowa <- aov(price ~ price_per_square_meter + has_balcony + has_parking, data = czestochowa)
#summary(ancova_czestochowa)

# Test Ancovy dla Radomia: 
ancova_radom <- aov(price ~ price_per_square_meter + has_parking, data = radom)
#summary(ancova_radom)

# Test Ancovy dla Krakowa: 
ancova_krakow <- aov(price ~ price_per_square_meter + has_balcony + has_elevator + has_parking + has_security + has_storage_room, data = krakow)
#summary(ancova_krakow)

# Test Ancovy dla Warszawy: 
ancova_warszawa <- aov(price ~ price_per_square_meter + has_balcony + has_elevator + has_parking + has_security + has_storage_room, data = warszawa)
#summary(ancova_warszawa)

# Test Ancovy dla Rzeszowa: 
ancova_rzeszow <- aov(price ~ price_per_square_meter + has_elevator + has_storage_room, data = rzeszow)
